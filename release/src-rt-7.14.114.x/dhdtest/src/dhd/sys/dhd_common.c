	...
	wl_event_msg_t *event, void **data_ptr, void *raw_event)
{
	...
	case WLC_E_DPSTA_INTF_IND: {
		...
		wl_dpsta_intf_event_t *dpsta_if = (wl_dpsta_intf_event_t *)event_data;

		if (dpsta_if->intf_type == WL_INTF_PSTA) {
			dhd_dpsta_psta_register(dhd_pub);
		}
...
#if defined(DHD_PSTA)
		wl_psta_create_event_t *psta_create_e = (wl_psta_create_event_t *)event_data;

		dhd_set_psta_ds_ea(dhd_pub,
			dhd_ifname2idx(dhd_pub->info, event->ifname),
			&psta_create_e->ds_ea,
			&psta_create_e->psta_ea);
#endif
		break;
	}
	case WLC_E_LINK:
