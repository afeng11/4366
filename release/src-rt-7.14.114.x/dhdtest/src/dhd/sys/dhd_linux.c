 ...
 *
 * $Id: dhd_linux.c 649538 2016-07-18 03:16:56Z $
 */
...
#endif /* BCM_ROUTER_DHD */
#ifdef DHD_PSTA
	uint8	ds_ea[ETHER_ADDR_LEN]; /* Original downstream ether addr. in PSTA mode */
#endif
} dhd_if_t;
...
#ifdef DHD_PSTA
/** Based on input mac to search ifp */
dhd_if_t *
dhd_get_ifp_by_mac(dhd_pub_t *dhdp, uint8 *mac, bool cmp_ds_ea)
{
	int i;
	...
	for (i = 0; i < DHD_MAX_IFS; i++)
	{
		if (dhdp->info->iflist[i]) {
			if (cmp_ds_ea) {
				/* In PSTA, ds_ea is original mac of client */
				if (!memcmp(dhdp->info->iflist[i]->ds_ea,
					mac, ETHER_ADDR_LEN)) {
					return dhdp->info->iflist[i];
				}
			} else {
				/* In PSTA, mac_addr is proxy mac of client */
...
/** Based on psta_ea to add original client MAC to dhd_if_t */
void
dhd_set_psta_ds_ea(dhd_pub_t *dhdp, int ifidx, struct ether_addr *ds_ea, struct ether_addr *psta_ea)
{
	dhd_if_t *ifp;

	ASSERT(psta_ea != NULL);

	ifp = dhd_get_ifp_by_mac(dhdp, psta_ea->octet, FALSE);

	if (ifp) {
		memcpy(ifp->ds_ea, ds_ea->octet, ETHER_ADDR_LEN);
	} else {
		DHD_ERROR(("%s: can't find ifidx %d\n", __func__, ifidx));
	}
}

/* Based on input mac to get mod_mac or cli_mac.
 * cli_mac: original client mac
 * mod_mac: modification (proxy) mac of client
 * get_mod: boolean value to define get mod_mac or cli_mac
 */
int
dhd_get_psta_mac(dhd_pub_t *dhdp, uint8 *cli_mac, uint8 *mod_mac, bool get_mod)
{
	dhd_if_t *ifp;
	uint8 *mac;
	...
	mac = get_mod ? cli_mac : mod_mac;

	ifp = dhd_get_ifp_by_mac(dhdp, mac, get_mod);

	if (ifp) {
		if (get_mod) {
			memcpy(mod_mac, ifp->mac_addr, ETHER_ADDR_LEN);
		} else {
			memcpy(cli_mac, ifp->ds_ea, ETHER_ADDR_LEN);
		}
	} else {
		DHD_INFO(("%s: fail to get %s_mac\n",
			__func__, get_mod ? "mod" : "cli"));
...
#if defined(DHD_PSTA) && defined(DHD_DPSTA)
	if (PSTA_ENABLED(dhdp)) {
		if (bcm_validate_dpsta()) {
...
#endif /* BCM_ROUTER_DHD && BCM_GMAC3 */
{
	 ...
	 * since dongle doesn't have complete payload
	 */
	if (PSR_ENABLED(&dhd->pub) && !(ifp->primsta_dwds)) {
		void *npkt;

		/* The data portion of skb may share by different devs.
		 * Try to duplicate pkt in case of PSR.
		 */
		if (!PKTISCHAINED(skb) &&
			(npkt = PKTDUP(dhd->pub.osh, pktbuf)) != NULL) {
			if (dhd_psta_proc(&dhd->pub, ifidx, &npkt, TRUE) < 0) {
				DHD_ERROR(("%s:%s: psta send proc npkt failed\n", __FUNCTION__,
					dhd_ifname(&dhd->pub, ifidx)));
				/* free npkt and keep pktbuf to dhd_sendpkt() */
				PKTFREE(dhd->pub.osh, npkt, FALSE);
			} else {
				/* send npkt and free original pktbuf */
				ret = dhd_sendpkt(&dhd->pub, ifidx, npkt);
				PKTFREE(dhd->pub.osh, pktbuf, FALSE);
				goto done;
			}
		} else {
			/* try original psta process */
...
#if (defined(BCM_ROUTER_DHD) && defined(BCM_GMAC3))
bool
dhd_info_fwder_capable(dhd_pub_t *dhdp)
{
	dhd_info_t *dhd = dhdp->info;

	if (dhd->fwdh != FWDER_NULL)
		return TRUE;
	else
		return FALSE;
}
...
#if defined(BCM_GMAC3) && defined(DHD_DPSTA)
	char *var;
#endif /* DHD_DPSTA */
...
typedef struct dhd_dummy_dev_priv {
	struct net_device_stats stats;
} dhd_dummy_dev_priv_t;

#define DHD_DUMMY_DEV_PRIV_SIZE                (sizeof(dhd_dummy_dev_priv_t))
#define DHD_DUMMY_DEV_STATS(dev)               (((dhd_dummy_dev_priv_t *)DEV_PRIV(dev))->stats)

static int
dhd_dummy_start(struct sk_buff *skb, struct net_device *dev)
{
...
static int
dhd_dummy_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	return 0;
}

static struct net_device_stats*
dhd_dummy_get_stats(struct net_device *dev)
{
	return &DHD_DUMMY_DEV_STATS(dev);
}

static const struct net_device_ops netdev_dummy_ops =
{
	.ndo_start_xmit = dhd_dummy_start,
	.ndo_get_stats = dhd_dummy_get_stats,
	.ndo_do_ioctl = dhd_dummy_ioctl
};

struct net_device *
dhd_register_dummy_if(char *ifname, bool need_rtnl_lock)
{
	struct net_device *dev = NULL;
	int err;
	...
	dev = alloc_etherdev(DHD_DUMMY_DEV_PRIV_SIZE);
	if (!dev) {
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 31))
	dev->hard_start_xmit = dhd_dummp_start;
	dev->do_ioctl = dhd_dummy_ioctl;
	dev->get_stats = dhd_dummy_get_stats;
#else
	dev->netdev_ops = &netdev_dummy_ops;
#endif
...
void
dhd_unregister_dummy_if(struct net_device *dev)
{
	if (dev) {
...
dhd_module_init(void)
{
	...
	{
		char * var;
                uint chipnum = 0, chiprev = 0;
 
                if ((var = getvar(NULL, "chipnum")) != NULL) {
                       chipnum = bcm_strtoul(var, NULL, 0);
                }
                DHD_ERROR(("chipnum = %#x\n", chipnum));

                if ((var = getvar(NULL, "chiprev")) != NULL) {
                        chiprev = bcm_strtoul(var, NULL, 0);
                }
                DHD_ERROR(("chiprev = %#x\n", chiprev));
 
                if (BCM4365_CHIP(chipnum) && CHIPREV(chiprev) < 4) {
                        DHD_ERROR(("%s: don't support chip %#x rev %#x\n",
                                __FUNCTION__, chipnum, chiprev));
...
static bool
dhd_psta_is_ds_sta(dhd_pub_t *dhdp, struct ether_addr *mac)
{
	bool ret;
	ret = dhd_find_sta(dhdp, 1, mac); /* ifidx of PSR AP interface is always 1 */
	return ret;
}

static bool
dhd_psta_find_dpsta(dhd_pub_t *dhdp, uint8 *mac)
{
	dhd_if_t *ifp = NULL;
	int i = 0;

	/* psta_if is start from wlx.2 */
	for (i = 2; i < DHD_MAX_IFS; i++) {
...
static bool
dhd_psta_authorized(dhd_pub_t *dhdp)
{
...
bcm_validate_dpsta(void)
{
	...
	if (dev == NULL) {
		DHD_ERROR(("%s: Interface %s doesn't exist\n",
				__FUNCTION__, if_name));
		return FALSE;
	}

	ret = (dev->priv_flags & IFF_BRIDGE_PORT);
...
void dhd_dpsta_psta_register(dhd_pub_t *dhd)
{
	...
	api.is_ds_sta = (bool (*)(void *, struct ether_addr *))dhd_psta_is_ds_sta;
	api.psta_find = (void *(*)(void *, uint8 *))dhd_psta_find_dpsta;
	api.bss_auth = (bool (*)(void *))dhd_psta_authorized;
	api.psta = dhd;
	api.bsscfg = dhd;
...
void dhd_dpsta_dwds_register(dhd_pub_t *dhd)
{
	...
	/* Register proxy sta APIs with DPSTA module */
	api.is_ds_sta = (bool (*)(void *, struct ether_addr *))dhd_psta_is_ds_sta;
	api.psta_find = (void *(*)(void *, uint8 *))NULL;
	api.bss_auth = (bool (*)(void *))dhd_psta_authorized;
	api.psta = dhd;
	api.bsscfg = dhd;
