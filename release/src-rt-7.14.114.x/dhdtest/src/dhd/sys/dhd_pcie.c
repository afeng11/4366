 ...
 *
 * $Id: dhd_pcie.c 665904 2016-10-19 10:09:44Z $
 */
...
dhdpcie_dongle_attach(dhd_bus_t *bus)
{
	...
	if (!(bus->sih = si_attach((uint)devid, osh, regsva, PCI_BUS, bus,
        ...
        if (BCM4365_CHIP(bus->sih->chip)) {
                sprintf(nv_value, "%#x", bus->sih->chip);
                nvram_set("chipnum", nv_value);
                sprintf(nv_value, "%#x", bus->sih->chiprev);
                nvram_set("chiprev", nv_value);
		nvram_set("dhd24", (CHIPREV(bus->sih->chiprev) < 4) ? "1": "0");
                nvram_commit();

		if(CHIPREV(bus->sih->chiprev) < 4) {
...
#ifdef DHD_DEBUG
#ifdef DUMP_CONSOLE_MEM
static int
dhdpcie_bus_dump_console_mem(dhd_bus_t *bus)
{
...
#ifdef DUMP_CONSOLE_MEM
			if ((dhdpcie_bus_dump_console_mem(bus)) == BCME_OK)
				goto printbuf;
#endif /* DUMP_CONSOLE_MEM */
