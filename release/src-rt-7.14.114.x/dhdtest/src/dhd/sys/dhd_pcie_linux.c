...
dhdpcie_pci_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	char buf[16] = { 0 };
	int buf_len = strlen("wlX_failed") + 1;
	char value[16] = { 0 };
	int fail_cnt = 0;
	int retry = 0;
	...
	snprintf(buf, buf_len, "wl%d_failed", dhd_found);
	fail_cnt = bcm_atoi(nvram_safe_get(buf));
	if (fail_cnt >= DHD_RETRY_MAX)
		goto fail;

	/* increase dhd failure count before we continue normal process */
	fail_cnt++;
	sprintf(value, "%d", fail_cnt);
	nvram_set(buf, value);
	nvram_commit();
	...
	/* clear dhd failure count nvram - wlX_failed */
	nvram_unset(buf);
	nvram_commit();
...
fail:
	...
	/* Assume dhdpcie_info_t is still null here if dhdpcie_init() return false */
	...
	dhd_found++;
	pch->dummy_if = dhd_register_dummy_if("eth%d", TRUE);
...
dhdpcie_pci_remove(struct pci_dev *pdev)
{
	...
	if (pch->dummy_if) {
		dhd_unregister_dummy_if(pch->dummy_if);
		pch->dummy_if = NULL;
...
dhdpcie_request_irq(dhdpcie_info_t *dhdpcie_info)
{
	...
	if (dhd_info_fwder_capable(bus->dhd))
