 ...
 *
 * $Id: dhd_msgbuf.c 665954 2016-10-19 12:04:19Z $
 */
...
#define WLC_GET_CLM_VER		310

#if defined(RTAC3100)
const char *model_name = "RT-AC3100";
#elif defined(RTAC88U)
const char *model_name = "RT-AC88U";
#elif defined(RTAC5300)
const char *model_name = "RT-AC5300";
#else
const char *model_name = "ASUS ROUTER";
#endif
const char *asus_clm_ver = " 1.15";
...
/*	don't lookfor nvram in the bottom-half
 ...
 */
        if(ioc->cmd == WLC_GET_VAR && buf) {
                if (!strcmp((char *)buf, "clm_data_ver")) {
                        strncpy((char *)buf, model_name, strlen(model_name));
                        strncpy((char *)buf+strlen(model_name), asus_clm_ver, strlen(asus_clm_ver));
                        strncpy((char *)buf+strlen(model_name)+strlen(asus_clm_ver), "\0", 1);
...
dhd_prot_runner_txstatus_process(dhd_pub_t *dhd, void* msg)
{
	...
	if (DHD_RNR_OFFL_TXSTS(dhd)) {
	    /* Runner based flow rings should never come here */
	    if (DHD_FLOWRING_RNR_OFFL(flow_ring_node)) {
