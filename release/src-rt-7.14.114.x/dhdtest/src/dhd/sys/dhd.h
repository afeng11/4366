 ...
 *
 * $Id: dhd.h 640237 2016-05-26 21:41:36Z $
 */
...
extern struct net_device *dhd_register_dummy_if(char *ifname, bool need_rtnl_lock);
extern void dhd_unregister_dummy_if(struct net_device *dev);
...
#ifdef DHD_PSTA
extern struct dhd_if *dhd_get_ifp_by_mac(dhd_pub_t *dhdp, uint8 *mac, bool cmp_ds_ea);
extern void dhd_set_psta_ds_ea(dhd_pub_t *dhdp, int ifidx, struct ether_addr *ds_ea, struct ether_addr *psta_ea);
extern int dhd_get_psta_mac(dhd_pub_t *dhdp, uint8 *cli_mac, uint8 *mod_mac, bool get_mod);
#endif
...
#if defined(BCM_ROUTER_DHD)
#if defined(BCM_GMAC3)
bool dhd_info_fwder_capable(dhd_pub_t *dhdp);
