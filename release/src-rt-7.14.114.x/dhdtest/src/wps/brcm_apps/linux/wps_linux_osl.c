 ...
 *
 * $Id: wps_linux_osl.c 527735 2015-01-20 02:56:21Z $
 */
...
wps_osl_arp_get(char *client_ip, unsigned char mac[6])
{
	...
	char titlestr[100];
	char line[100];
	char *ipaddrstr, *hwtypestr, *flagstr,
		*hwaddrstr, *maskstr, *devicestr;
	...
	*/
	while (fgets(line, 100, arpfile))
	{
		ipaddrstr = strtok(line, "\t ");
		hwtypestr = strtok(NULL, "\t ");
		flagstr = strtok(NULL, "\t ");
		hwaddrstr = strtok(NULL, "\t ");
		maskstr = strtok(NULL, "\t ");
