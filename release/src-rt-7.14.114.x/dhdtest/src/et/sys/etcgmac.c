 ...
 * $Id: etcgmac.c 529627 2015-01-28 03:57:01Z $
 */
...
chipattach(etc_info_t *etc, void *osh, void *regsva)
{
	...
	if (etc->unit == 1 &&
	   (BCM53573_CHIP(ch->sih->chip)) && (ch->sih->chippkg == BCM53573_PKG_ID)) {
		ET_ERROR(("et%d: chipattach: Do not init gmac1 in 53573 package\n", etc->unit));
		goto fail;
	}
...
chipinreset:
	...
	if (BCM53573_CHIP(ch->sih->chip) && (PMUCTL_ENAB(ch->sih))) {
		...
		ET_TRACE(("et%d: 53573 sw_type %04x\n", ch->etc->unit, sw_type));
	}
...
#ifndef _CFE_
	if (BCM4707_CHIP(CHIPID(ch->sih->chip)) &&
			(OSL_ACP_WAR_ENAB() || OSL_ARCH_IS_COHERENT())) {
...
chipphyinit(ch_t *ch, uint phyaddr)
{
	...
	if ((BCM53573_CHIP(ch->sih->chip) &&
		...
		/* ephy should be the same as 5357.  use the same setup sequence */
		for (i = 0; i < 5; i++) {
			chipphywr(ch, i, 0x0, 0x8000);		/* reset ephy */
			chipphywr(ch, i, 0x1f, 0x000f);		/* enable the shadow register */
			chipphywr(ch, i, 0x13, 0x5555);		/* disable the inverted adc clock */
			chipphywr(ch, i, 0x1f, 0x000b);		/* return */
		}
	}
