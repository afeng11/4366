 ...
 *
 * $Id: wlc_clm.c 529529 2015-01-27 21:46:24Z $
 */
...
#define FORMAT_VERSION_MAJOR 16
...
typedef struct data_dsc {
	...
	/** True if BLOB contains ULB channels */
	MY_BOOL has_ulb;
...
static data_dsc_t data_sources[] = {
	...
	(clm_ccrev_format_t)0, {{NULL}}, 0, NULL },
	{ NULL, 0, {{{0, 0}}}, 0, 0, 0, NULL, 0, {NULL}, {{NULL}}, 0, 0, 0,
	(clm_ccrev_format_t)0, {{NULL}}, 0, NULL }
};
 ...
 * flags
 * \param[out] base_ht_loc_data Container for base locale helper information
...
try_fill_valid_channel_combs(data_source_id_t ds_id)
{
	...
	if (ds->has_high_bw_combs) {
		...
		int idx;
		for (idx = 0; idx < ARRAYSIZE(fields); ++idx) {
			if (!is_ulb_bw(fields[idx].bw) || ds->has_ulb) {
...
data_init(const clm_data_header_t *header, data_source_id_t ds_id)
{
	...
	if (header) {
		...
		ds->has_160mhz = has_registry_flags &&
				(ds->data->flags & CLM_REGISTRY_FLAG_160MHZ);
		if (has_registry_flags && (ds->data->flags & CLM_REGISTRY_FLAG_PER_BW_RS)) {
			...
			if (ds->data->flags & CLM_REGISTRY_FLAG_EXT_RATE_SETS) {
				...
				int idx;
				for (idx = 0; idx < ARRAYSIZE(ext_rate_sets); ++idx) {
		...
		ds->has_high_bw_combs = has_registry_flags &&
			(ds->data->flags & CLM_REGISTRY_FLAG_HIGH_BW_COMBS);
		ds->has_ulb = has_registry_flags &&
			(ds->data->flags & CLM_REGISTRY_FLAG_ULB);
	} else {
		ds->relocation = 0;
		ds->data = NULL;
	}
	...
	fill_rate_types();
#ifdef WL11ULB
	bw_to_ulb[CLM_BW_20] = CLM_BW_2_5;
	bw_to_ulb[CLM_BW_40] = CLM_BW_5;
	bw_to_ulb[CLM_BW_40 + 1] = CLM_BW_10;
#endif /* WL11ULB */
	...
	int *num_aggregates)
{
	...
	if (maps_indices) {
		...
		result->regions = ((const clm_aggregate_cc16_t *)p)->regions;
	} else {
		...
		result->regions = ((const clm_aggregate_cc_t *)p)->regions;
	}
	return TRUE;
}
	...
	const ccode_t cc, clm_cc_rev4_t *result)
{
	const unsigned char *mappings =
			agg ? (const unsigned char *)relocate_ptr(ds_id, agg->regions) : NULL;
...
fill_actual_subchan_table(unsigned char actual_table[CLM_BW_NUM],
	...
	const clm_channel_comb_set_t *comb_set, const void *rules,
	int num_rules, int rule_len)
{
	/* Rule pointer as character pointer (to simplify address
	 * arithmetic)
	 */
	const unsigned char *r;
	/* Converting subchannel ID from clm_limits_type_t to
	...
	for (r = (const unsigned char *)rules; num_rules--; r += rule_len) {
		...
		if (channel_in_range(channel, ranges + r[CLM_SUB_CHAN_RANGE_IDX], comb_set, 0)) {
			...
			for (bw_idx = 0; bw_idx < CLM_BW_NUM; ++bw_idx) {
				...
				if ((1 << (bw_idx - CLM_BW_20)) & sub_chan_rule) {
	...
	country_data_t *country_data)
{
	...
	if (locales->country_flags & CLM_DATA_FLAG_REG_SC_RULES_MASK) {
		int rules_idx = remove_extra_bits(
				locales->country_flags & CLM_DATA_FLAG_REG_SC_RULES_MASK,
				CLM_DATA_FLAG_REG_SC_RULES_EXTRA_BITS) - CLM_SUB_CHAN_IDX_BASE;
		const clm_sub_chan_rules_set_80_t *sub_chan_rules_set_80 =
				(const clm_sub_chan_rules_set_80_t *)GET_DATA(ds_id,
				sub_chan_rules_80);
		if (sub_chan_rules_set_80 && ((unsigned)rules_idx <
			...
			const clm_sub_chan_region_rules_80_t *sub_chan_region_rules =
					(const clm_sub_chan_region_rules_80_t*)
					relocate_ptr(ds_id, sub_chan_rules_set_80->region_rules)
					+ rules_idx;
			country_data->sub_chan_channel_rules_80.channel_rules =
					(const clm_sub_chan_channel_rules_80_t *)
					relocate_ptr(ds_id, sub_chan_region_rules->channel_rules);
	...
	const clm_limits_params_t *params, clm_power_limits_t *limits)
{
	...
	* country
	*/
	country_comb_sets = get_data_sources((data_source_id_t)(locales->computed_flags &
			COUNTRY_FLAGS_DS_MASK))->valid_channel_combs[band];
#endif /* WL11AC */
	...
	for (base_ht_idx = 0; base_ht_idx < (int)ARRAYSIZE(base_ht_loc_data); ++base_ht_idx) {
		...
		do {
...
#ifdef WL11ULB
			if (flags2 & CLM_DATA_FLAG2_WIDTH_EXT) {
				pg_bw = (clm_bandwidth_t)bw_to_ulb[(unsigned char)pg_bw];
			}
#endif /* WL11ULB */
			...
			for (num_rec = (int)*tx_rec++; num_rec--; tx_rec += tx_rec_len)
			{
				...
				/* Power target for current transmission power
				 * record
				 */
				char qdbm;
