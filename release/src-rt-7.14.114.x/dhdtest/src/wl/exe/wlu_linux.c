 ...
 *
 * $Id: wlu_linux.c 656094 2016-08-25 06:37:41Z $
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
int is_dbg_usage(char *name)
{
        int i = 0;
        int dbg_on = atoi(nvram_safe_get("debug_wl"));

        for (i = 0; dbgcmd_list[i] != NULL ; i++) {
                if (!strncmp(dbgcmd_list[i], name, strlen(dbgcmd_list[i]))) {
                        if(dbg_on != 1) {
                                return 1;
                        }
                }
