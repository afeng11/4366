 ...
 *
 * $Id: wlu.c 654912 2016-08-17 06:22:26Z $
 */
...
cmd_t wl_cmds[] = {
	...
	"\t\t184, 188, 192, 196, 200, 204, 208, 212, 216"},
        { "clm_data_ver", wl_dump_clm_ver, 310, -1,
...
extern int is_dbg_usage(char *name);
...
int
wl_dump_clm_ver(void *wl, cmd_t *cmd, char **argv)
{
        printf("%s %s \n", model_name, asus_clm_ver);
...
wl_counters(void *wl, cmd_t *cmd, char **argv)
{
	...
	void *ptr;
	uint16 ver;
	uint8 cntdata[WL_CNTBUF_MAX_SIZE * 2];
	uint32 corerev = 0;
	...
	if (!argv[1]) {
		...
		do {
			...
			if (!strcmp(s, "-h")) {
				...
				val = strtoul(*argv++, NULL, 0);
				if ((val >= WL_MU_POLICY_MAXCLIENTS_MIN) &&
					(val <= WL_MU_POLICY_MAXCLIENTS_MAX)) {
