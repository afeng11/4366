...
static cmd_t wl_phy_cmds[] = {
	...
	"\tmin_burst_intv_lp, max_burst_intv_lp, nskip_rst_lp, max_pw_tol, feature_mask,\n"
	"\tthresh0_sc, thresh1_sc"},
	{ "radarthrs", wl_radar_thrs, WLC_GET_VAR, WLC_SET_VAR,
...
wl_radar_thrs(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (*++argv) {
		...
		return wlu_var_setbuf(wl, cmd->name, &radar_thrs, sizeof(wl_radar_thr_t));
                ...
                /* Get */
                if ((ret = wlu_iovar_get(wl, cmd->name, &radar_thrs, sizeof(radar_thrs))) < 0)
                        return ret;
 
                if (radar_thrs.version != WL_RADAR_ARGS_VERSION) {
                        printf("\tIncorrect version"
                        "\tof RADAR_ARGS struct:expected %d; got %d\n",
                        WL_RADAR_ARGS_VERSION, radar_thrs.version);
                        return -1;
                }
                printf("version %d\n"
                "thresh0_20_lo 0x%x thresh1_20_lo 0x%x"
                "thresh0_40_lo 0x%x thresh1_40_lo 0x%x"
                "thresh0_80_lo 0x%x thresh1_80_lo 0x%x\n"
                "thresh0_20_hi 0x%x thresh1_20_hi 0x%x"
                "thresh0_40_hi 0x%x thresh1_40_hi 0x%x"
                "thresh0_80_hi 0x%x thresh1_80_hi 0x%x\n",
                radar_thrs.version,
                radar_thrs.thresh0_20_lo,
                radar_thrs.thresh1_20_lo,
                radar_thrs.thresh0_40_lo,
                radar_thrs.thresh1_40_lo,
                radar_thrs.thresh0_80_lo,
                radar_thrs.thresh1_80_lo,
                radar_thrs.thresh0_20_hi,
                radar_thrs.thresh1_20_hi,
                radar_thrs.thresh0_40_hi,
                radar_thrs.thresh1_40_hi,
                radar_thrs.thresh0_80_hi,
                radar_thrs.thresh1_80_hi
		);
	}
	return ret;
}
