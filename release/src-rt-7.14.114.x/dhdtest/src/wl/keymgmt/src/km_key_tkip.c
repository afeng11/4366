 ...
 * $Id: km_key_tkip.c 523118 2014-12-26 18:53:31Z $
 */
	...
	uint8 *body, int body_len, wlc_txd_t *txd)
{
	...
	if (WLC_KEY_IN_HW(&key->info) && txd != NULL) {
		...
		if (KEY_USE_AC_TXD(key)) {
			...
			if (D11REV_GE(key->wlc->pub->corerev, 64)) {
				ph1 = txh->u.rev64.CacheInfo.TkipPH1Key;
				pn = txh->u.rev64.CacheInfo.TSCPN;
			} else {
				ph1 = txh->u.rev40.CacheInfo.TkipPH1Key;
				pn = txh->u.rev40.CacheInfo.TSCPN;
			}
			if (hwmic)
