 ...
 * $Id: km_hw.c 523118 2014-12-26 18:53:31Z $
 */
...
BCMATTACHFN(km_hw_attach)(wlc_info_t *wlc, keymgmt_t *km)
{
	...
	STATIC_ASSERT(AMT_SIZE >= RCMTA_SIZE);
	...
	if (KM_HW_COREREV_GE40(hw)) {
		STATIC_ASSERT((AMT_SIZE + 4) == M_SECKINDXALGO_BLK_SZ);

		hw->max_amt_idx = AMT_SIZE;
		hw->max_idx =  AMT_SIZE;
		...
		STATIC_ASSERT((RCMTA_SIZE + 4) == D11_PRE40_M_SECKINDXALGO_BLK_SZ);
	...
	if (KM_HW_COREREV_GE40(hw))
		hw->shm_info.skl_idx_base = M_SECKINDXALGO_BLK;
	else
		 hw->shm_info.skl_idx_base = D11_PRE40_M_SECKINDXALGO_BLK;

	hw->shm_info.tkip_tsc_ttak_base = M_TKIP_TSC_TTAK;
	hw->shm_info.max_tsc_ttak = KM_HW_TKIP_MAX_TSC_TTAK;
	if (D11REV_GE(KM_HW_COREREV(hw), 13) && (wlc->machwcap & MCAP_TKIPMIC)) {
...
km_hw_reset(km_hw_t *hw)
{
	...
	memset(&hw->amt_info.used, 0, sizeof(hw->amt_info.used));
	...
	if (PSTA_ENAB(KM_HW_PUB(hw))) {
		...
		km_hw_amt_reserve(hw, PSTA_TA_STRT_INDX, PSTA_RA_PRIM_INDX, FALSE);
		km_hw_amt_reserve(hw, PSTA_RA_PRIM_INDX, 1, FALSE);
	}
#endif
	...
	wlc_key_t *key, const wlc_key_info_t *key_info)
{
...

	{
		...
		skl_val &= ~(SKL_ALGO_MASK | SKL_INDEX_MASK | SKL_KEYID_MASK);
		skl_val |= (skl_hw_algo << SKL_ALGO_SHIFT) & SKL_ALGO_MASK;
		skl_val |= (skl_hw_idx << SKL_INDEX_SHIFT) & SKL_INDEX_MASK;

		if (WLC_KEY_IS_GROUP(key_info) &&
			KM_HW_BSSCFG_NEED_STA_GROUP_KEYS(hw, bsscfg)) {
			skl_val &= ~SKL_GRP_ALGO_MASK;
			skl_val |= (skl_hw_algo << SKL_GRP_ALGO_SHIFT) & SKL_GRP_ALGO_MASK;
		}
