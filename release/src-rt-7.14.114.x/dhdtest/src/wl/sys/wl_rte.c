 ...
 *
 * $Id: wl_rte.c 529317 2015-01-27 05:44:17Z $
 */
...
#ifdef BCMPCIEDEV
static uint16 wl_flowring_update(hnd_dev_t *dev, uint16 flowid, uint8 op, uint8 * sa,
	uint8 *da, uint8 tid);
#endif /* BCMPCIEDEV */
...
wl_monitor(wl_info_t *wl, wl_rxsts_t *rxsts, void *p)
{
	...
	len = PKTLEN(wl->pub->osh, p) - D11_PHY_HDR_LEN;
...
wl_alloc_if(wl_info_t *wl, int iftype, uint subunit, struct wlc_if *wlcif, bool rebind)
{
	...
	if (iftype == WL_IFTYPE_WDS) {
		(void)snprintf(dev->name, HND_DEV_NAME_MAX, "wds%d.%d", wl->pub->unit, subunit);
	} else {
                      ...
                      uint coreid, uint unit)
{
	...
	*/
	...
	/* We set up the event and start the tracing immediately */
	wlc_eventq_set_ind(wlc->eventq, WLC_E_TRACE, TRUE);
	if (logtrace_init(wlc->osh, wlc, wl->dev, (msgtrace_func_send_t)wlc_event_sendup_trace) !=
		BCME_OK) {
		goto fail;
	}
	logtrace_start();
...
#ifdef BCMPCIEDEV
	if (WLFC_INFO_TO_BUS_ENAB(wl->pub)) {
...
wl_pkt_header_push(wl_info_t *wl, void *p, uint8 *wl_hdr_words)
{
	...
	*wl_hdr_words = WL_HEADER_LEN/RTE_WORD_SIZE;
	return p;
}
...
#endif /* WLWNM_AP */
	    (PSTA_ENAB(wl->pub) && PSTA_IS_REPEATER(wlc) && BSSCFG_STA(bsscfg)) ||
	    CAC_ENAB(wl->pub) ||
...
wl_sw_tkip_mic_enab(wl_info_t *wl, struct wlc_if *wlcif, wlc_bsscfg_t *bsscfg, struct lbuf *lb)
{
	...
	if ((key_info.algo == CRYPTO_ALGO_TKIP &&
		!WLC_KEY_MIC_IN_HW(&key_info)) || WLPKTFLAG_PMF(WLPKTTAG(lb)))
		tkip_enab = TRUE;
...
wl_tx_pktfetch_required(wl_info_t *wl, wl_if_t *wlif, wlc_bsscfg_t *bsscfg, struct lbuf *lb)
{
	...
	if (WSEC_ENABLED(bsscfg->wsec) && wl_sw_tkip_mic_enab(wl, wlcif, bsscfg, lb))
		return TRUE;
...
#ifdef PKTC_TX_DONGLE
		if (PKTC_ENAB(wl->pub)) {
			chainable = wl_txframe_chainable(wl, bsscfg, p, head);
