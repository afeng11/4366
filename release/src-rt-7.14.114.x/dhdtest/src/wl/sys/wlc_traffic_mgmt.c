 ...
 *
 * $Id: wlc_traffic_mgmt.c 530263 2015-01-29 18:21:14Z $
 */
...
trf_mgmt_scbcubby_init(void *handle, struct scb *scb)
{
	...
	if (trf_mgmt_rssi_needed(tm)) {
		...
		if (!trf_mgmt_is_enabled(scb)) {
			wlc_txmod_config(tm->wlc->txmodi, scb, TXMOD_TRF_MGMT);
		}
	}
	return BCME_OK;
}
...
BCMATTACHFN(wlc_trf_mgmt_attach)(wlc_info_t *wlc)
{
	...
	wlc_txmod_fn_register(wlc->txmodi, TXMOD_TRF_MGMT, trf_mgmt_ctxt, trf_mgmt_txmod_fns);
	...
	if (BSSCFG_STA(bsscfg)) {
		...
		if (PSTA_ENAB(bsscfg->wlc->pub)) {
			...
			FOREACH_BSS_SCB(
				...
				if (!SCB_IS_IBSS_PEER(scb)) {
					wlc_txmod_config(trf_mgmt_ctxt->wlc->txmodi,
					                 scb, TXMOD_TRF_MGMT);
				}
			}
		} else
#endif /* PSTA */
		{
			...
			*/
			...
			wlc_txmod_config(trf_mgmt_ctxt->wlc->txmodi, scb, TXMOD_TRF_MGMT);
		}
	}
}
	...
	uint32                  cmd_length)
{
	...
	if (cmd_length <
		...
		WL_ERROR(("wl%d: %s: Invalid cmd_length %d, buffer size %d \n",
	...
	wlc_pkt_desc_t      *pkt_desc)
{
	...
	if (TRF_MGMT_DWM_FILTER_ENAB(trf_mgmt_info) &&
		...
		if (pkt_desc && pkt_desc->dscp >= DWM_TBL_SIZE) {
			WL_TRF_MGMT(("Invalid DSCP value 0x%x in packet\n", pkt_desc->dscp));
			return BCME_ERROR;
		}
	...
	} else if (!TRF_MGMT_DWM_FILTER_ENAB(trf_mgmt_info)) {
	    /*
	     * Legacy trf mgmt sets BE priority for all packets that do not match any
	     * legacy filter. This conflicts with DWM requirement requiring keeping priority
	     * as assigned by driver if no DWM filter match.
	     * Only if no DWM filter is configured and legacy trf mgmt filter is configured
	     * but doesn't match the current packet will packet priority be set to BE,
	     * otherwise, the packet will keep its default priority as assigned by the driver.
	     */
	    priority_class = trf_mgmt_priority_medium;
	}
