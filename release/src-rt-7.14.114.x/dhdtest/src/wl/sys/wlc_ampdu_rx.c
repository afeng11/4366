 ...
 *
 * $Id: wlc_ampdu_rx.c 529218 2015-01-26 21:13:23Z $
 */
...
#ifdef WLC_HIGH_ONLY
		/* reduce rx_factor from 1024K to 128K for BMAC USB2 devices */
		if (!(wlc->pub->is_ss)) {
			ampdu_rx_cfg->rx_factor = AMPDU_RX_FACTOR_128K;
		}
#endif /* WLC_HIGH_ONLY */
	} else
#endif /* WL11AC */
...
wlc_ampdu_rx_down(void *hdl)
{
	...
	if (WOWL_ACTIVE(ampdu_rx->wlc->pub))
		return 0;
...
wlc_ampdu_recv_bar(ampdu_rx_info_t *ampdu_rx, struct scb *scb, uint8 *body, int body_len)
{
	...
	if (AMPDU_HOST_REORDER_ENAB(ampdu_rx->wlc->pub) && resp->queued) {
		if (BCMPCIEDEV_ENAB()) {
			...
			if (p == NULL) {
				/* what to do...fine if there is going to be more packets from
				 * peer
				 */
				return;
			}
	...
	if (AMPDU_HOST_REORDER_ENAB(ampdu_rx->wlc->pub)) {
		if (BCMPCIEDEV_ENAB()) {
			...
			WLPKTTAGBSSCFGSET(p, scb->bsscfg->_idx);
			wlc_sendup(ampdu_rx->wlc, scb->bsscfg, scb, p);
		}
	}
#endif /* WLAMPDU_HOSTREORDER */
} /* wlc_ampdu_recv_bar */
		...
		if (forward) {
			...
			if (ptr->new_hole_informed) {
				...
				if ((p_ahr = PKTGET(wlc->osh, TXOFF, TRUE)) != NULL) {
					...
					wlc_sendup(wlc, bsscfg, NULL, p_ahr);
				}
...
void
wlc_ampdu_rxrates_get(ampdu_rx_info_t *ampdu_rx, wifi_rate_stat_t *rate, int i, bool vht)
{
	if (vht) {
		if (i < AMPDU_MAX_VHT)
