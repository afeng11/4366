 ...
 *
 * $Id: wlc_assoc.c 530263 2015-01-29 18:21:14Z $
 */
...
static void wlc_join_ibss(wlc_bsscfg_t *cfg, wlc_bss_info_t *bi);
static void wlc_join_BSS_sendauth(wlc_bsscfg_t *cfg, wlc_bss_info_t *bi,
	struct scb *scb);
static void wlc_join_BSS_select(wlc_bsscfg_t *cfg, wlc_bss_info_t *bi,
...
#if AS_LAST_STATE != 28 /* don't change this without adding to the table below!!! */
	...
	const struct ether_addr *bssid, const chanspec_t *chanspec_list, int channel_num)
{
	...
	if (ASSOC_CACHE_ASSIST_ENAB(wlc) && wlc->scan_results->count > 0) {
		...
		*/
		...
		if (as->type == AS_ROAM) {
			/* override source MAC */
			sa_override = wlc_scanmac_get_mac(wlc->scan, WLC_ACTION_ROAM);
		}
...
wlc_assoc_scan_complete(void *arg, int status, wlc_bsscfg_t *cfg)
{
	...
	if (roam->prev_scan_home_time) {
		WL_ROAM(("Restore home scan time to %d\n", roam->prev_scan_home_time));
		wlc_iovar_setint(wlc, "scan_home_time", roam->prev_scan_home_time);
		roam->prev_scan_home_time = 0;
	}
...
#ifdef WLRCC
			if (force_rescan) {
				if (roam->reason == WLC_E_REASON_BCNS_LOST) {
					int home_time;
					wlc_iovar_getint(wlc, "scan_home_time", &home_time);
					roam->prev_scan_home_time = home_time;
					WL_ROAM(("Reset scan home time to 0 from %d\n", home_time));
					wlc_iovar_setint(wlc, "scan_home_time", 0);
				}
	...
	if (bi->flags & WLC_BSS_HT) {
		...
		if (!WSEC_AES_ENABLED(cfg->wsec)) {
			if (WSEC_TKIP_ENABLED(cfg->wsec)) {
				if (wlc_ht_get_wsec_restriction(wlc->hti) & WLC_HT_TKIP_RESTRICT)
					ht_wsec = FALSE;
...
static void
wlc_join_ibss(wlc_bsscfg_t *cfg, wlc_bss_info_t *bi)
{
	wlc_info_t *wlc = cfg->wlc;
	wlc_assoc_t *as = cfg->assoc;
	/* IBSS join */
...
static void
wlc_join_BSS_select(wlc_bsscfg_t *cfg, wlc_bss_info_t *bi,
		chanspec_t chanspec, uint8 mcsallow, bool bss_ht, bool bss_vht)
{
	wlc_info_t *wlc = cfg->wlc;
	wlc_assoc_t *as = cfg->assoc;
	...
	if (!(scb->state & (PENDING_AUTH | PENDING_ASSOC))) {
		wlc_join_BSS_sendauth(cfg, bi, scb);
	}
}
...
wlc_join_BSS(wlc_bsscfg_t *cfg, wlc_bss_info_t *bi)
{
	...
	if (((WLC_BAND_PI_RADIO_CHANSPEC != chanspec) &&
		!(WLC_SAME_CTLCHAN(WLC_BAND_PI_RADIO_CHANSPEC, chanspec) &&
		CHSPEC_BW(WLC_BAND_PI_RADIO_CHANSPEC) > CHSPEC_BW(chanspec))) &&
		((as->type != AS_ROAM) ||
#ifdef WLFBT
	...
	if (bi->infra == 0) {
		wlc_join_ibss(cfg, bi);
	} else {
		wlc_join_BSS_select(cfg, bi, chanspec, mcsallow,
			bss_ht, bss_vht);
	}
...
wlc_join_start_ibss(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	int8 infra = wlc->default_bss->infra;
	uint8 dtim = wlc->default_bss->dtim_period;
	int err;
