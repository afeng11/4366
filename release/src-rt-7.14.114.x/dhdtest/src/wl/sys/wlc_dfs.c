 ...
 *
 * $Id: wlc_dfs.c 542259 2015-03-19 08:00:10Z $
 */
...
struct wlc_dfs_info {
	...
	uint32 radar_info;		/* radar info: radar type, min_pw .. */
};
...
wlc_radar_detected(wlc_dfs_info_t *dfs)
{
	...
	radar_type = phy_radar_detect_run((phy_info_t *)WLC_PI(wlc));
...
wlc_dfs_cacstate_init(wlc_dfs_info_t *dfs)
{
	...
	WL_REGULATORY(("wl%d: %s: state to %s chanspec %s \n",
		wlc->pub->unit, __FUNCTION__,
		wlc_dfs_cacstate_str[dfs->dfs_cac.status.state],
		wf_chspec_ntoa_ex(WLC_BAND_PI_RADIO_CHANSPEC, chanbuf)));
}
...
wlc_dfs_send_event(wlc_dfs_info_t *dfs, chanspec_t target_chanspec)
{
	...
	radar_data.radar_info[0].min_pw = RADAR_INFO_EXTRACT(dfs->radar_info, MINPW);
	radar_data.radar_info[0].subband = RADAR_INFO_EXTRACT(dfs->radar_info, SUBBAND);
	radar_data.radar_info[0].radar_type = RADAR_INFO_EXTRACT(dfs->radar_info, TYPE);
	wlc_bss_mac_event(wlc, wlc->cfg, WLC_E_RADAR_DETECTED, NULL, 0,
		0, 0, (void *)&radar_data, sizeof(radar_data));
	WL_DFS(("wl%d: DFS WLC_E_RADAR_DETECTED %04x on ch 0x%02x going to ch 0x%02x\n",
		WLCWLUNIT(wlc), dfs->radar_info,
