 ...
 *
 * $Id: wlc_bmac.c 530150 2015-01-29 08:43:40Z $
 */
...
#define D11MAC_BMC_TPL_BYTES_PERCORE   (12 * 1024) /* 12K Template bytes */
#else
...
#endif /* BCMDBG */
static void wlc_bmac_btc_param_attach(wlc_info_t *wlc);
...
#ifdef AMPDU_RX_BA_DEF_WSIZE
	revinfo->ampdu_ba_rx_wsize = AMPDU_RX_BA_DEF_WSIZE;
#else
			   ...
			   if ((WL_TXSTATUS_GET_FLAGS(whinfo) & WLFC_PKTFLAG_PKTFROMHOST) &&
				...
				for (p2 = p; processed && p2; p2 = PKTNEXT(osh, p2)) {
					...
					if (whinfo) {
						 ...
						 */
						if (WLFC_CONTROL_SIGNALS_TO_HOST_ENAB
							(wlc_hw->wlc->pub) &&
...
BCMATTACHFN(wlc_bmac_attach_dmapio)(wlc_hw_info_t *wlc_hw, bool wme)
{
	...
	if (PIO_ENAB_HW(wlc_hw)) {
		...
		di = dma_attach(osh, name, wlc_hw->sih,
			...
			(wme ? ntxd : 0), nrxd, tune->rxbufsz, extraheadroom, tune->nrxbufpost,
...
good_phy:
	...
	wlc_hw->templatebase = D11MAC_BMC_TPL_BYTES_PERCORE * wlc_hw->macunit;
...
#ifdef WLC_LOW
#if !defined(BCMDONGLEHOST) && !defined(PCIE_PHANTOM_DEV) && !defined(WL_PROXDETECT)
...
BCMINITFN(wlc_coreinit)(wlc_hw_info_t *wlc_hw)
{
	...
	*/
	...
	WL_ERROR(("wl%d: CORE INIT : mode %d pktclassify %d rxsplit %d  hdr conve %d \n",
...
#endif /* NOT_YET */
			if (D11REV_IS(wlc_hw->corerev, 55)) {
...
wlc_bmac_uflush_tx_fifos(wlc_hw_info_t *wlc_hw, uint fifo_bitmap)
{
	...
	*/
	for (i = 0, fbmp = fifo_bitmap; fbmp; i++, fbmp = fbmp >> 1) {
		...
		wlc_bmac_write_shm(wlc_hw, M_TXFL_BMAP, (1 << i)); /* ucode starts flushing now */
...
wlc_bmac_flush_tx_fifos(wlc_hw_info_t *wlc_hw, uint fifo_bitmap)
{
	...
	if (D11REV_GE(wlc_hw->corerev, 40)) {
		...
		for (i = 0, fbmp = fifo_bitmap; fbmp; i++, fbmp = fbmp >> 1) {
			...
			dma_txsuspend(wlc_hw->di[i]);
			/* request ucode flush */
			wlc_bmac_write_shm(wlc_hw, M_TXFL_BMAP, (uint16)(i << 1));
...
wlc_bmac_obss_stats_read(wlc_hw_info_t *wlc_hw, wlc_bmac_obss_counts_t *obss_counts)
{
	...
	if (D11REV_GE(wlc_hw->corerev, 40)) {
		...
		obss_counts->rx20s = wlc_bmac_read_shm(wlc_hw,
			M_UCODE_MACSTAT1 + OFFSETOF(macstat1_t, rx20s));
	}
#endif /* WL_PROT_OBSS */
...
wlc_bmac_gpio_configure(wlc_hw_info_t *wlc_hw, bool is_uppath)
{
	 ...
	 */
	if ((wlc_hw->sih->boardvendor == VENDOR_APPLE) &&
	    (wlc_hw->sih->boardtype == BCM943602X87 ||
	     wlc_hw->sih->boardtype == BCM943602X87P2 ||
	     wlc_hw->sih->boardtype == BCM943602X238 ||
	     wlc_hw->sih->boardtype == BCM943602X238D)) {
		if (!is_uppath) {
			/* Set to Input Mode */
