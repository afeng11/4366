 ...
 *
 * $Id: wlc_stf.c 624211 2016-03-10 23:10:35Z $
 */
...
wlc_stf_txcore_get(wlc_info_t *wlc, ratespec_t rspec)
{
	...
	/* Always use all currently enabled txcores if beamforming is enabled */
		idx = (uint8)(OFDM_IDX + wlc->stf->txstreams);
...
wlc_stf_txcore_shmem_write(wlc_info_t *wlc, bool forcewr)
{
	...
	if (D11REV_GE(wlc->pub->corerev, 40))
	/* M_COREMASK_BFM(M_COREMASK_BLK + 4) is filled up by TxBF */
		max_coremask_blk = MAX_COREMASK_BLK - 1;
	else
...
wlc_stf_get_204080_pwrs(wlc_info_t *wlc, ratespec_t rspec, txpwr204080_t* pwrs)
{
	...
	wl_tx_nss_t nss = wlc_ratespec_nss(rspec);
	wl_tx_chains_t chains = wlc_stf_txchain_get(wlc, rspec);
	wl_tx_chains_t txbf_chains = WL_TX_CHAINS_1;
	uint i, j;
	wl_tx_bw_t min_bw = WL_TX_BW_20;
#if defined(WL_BEAMFORMING)
	/* Ignore the spatial_expansion policy if beamforming is enabled
	 * Always use all currently enabled txcores
	 */
	if (TXBF_ENAB(wlc->pub))
		txbf_chains = wlc->stf->txstreams;
#endif
	...
	memset(pwrs, (uint8)WL_RATE_DISABLED, sizeof(*pwrs));
...
wlc_set_pwrthrottle_config(wlc_info_t *wlc)
{
	...
	is4360 = (wlc->pub->sih->boardvendor == VENDOR_APPLE &&
	          ((wlc->pub->sih->boardtype == BCM94360X29C) ||
	           ...
	           (wlc->pub->sih->boardtype == BCM943602X238D) ||
