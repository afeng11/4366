 ...
 *
 * $Id: wlc_rsdb.c 529832 2015-01-28 11:29:22Z $
 */
...
#if defined(BCMDBG) || defined(BCMDBGASSERT_LOG) || (defined(BCMDBG_TRAP) && \
	!defined(DONGLEBUILD))
...
#ifdef WL_MODESW
	if (WLC_MODESW_ENAB(wlc->pub) && (wlc->cmn->rsdb_mode & WLC_RSDB_MODE_AUTO_MASK)) {
...
#if defined(WL_MODESW) && defined(AP)
					/* Clear the AP updgrade pending flag */
					if (WLC_MODESW_ENAB(wlc->pub)) {
						wlc_set_ap_up_pending(wlc, cfg, FALSE);
					}
#endif
	...
	FOREACH_BSS(wlc, idx, bsscfg) {
		if (WLC_BSS_CONNECTED(bsscfg)) {
			...
			if ((nss == 1) && (wlc->stf->hw_rxchain_cap > 1))	{
				...
				WLC_BITSCNT(wlc->stf->hw_rxchain_cap), bw);
			}
			else {
			...
			/* Use the Rx max streams for both Rx and Tx */
			new_oper_mode = DOT11_OPER_MODE(0,
			WLC_BITSCNT(wlc->stf->hw_rxchain_cap), DOT11_OPER_MODE_80MHZ);
			wlc_modesw_handle_oper_mode_notif_request(wlc->modesw, bsscfg,
				new_oper_mode, TRUE, 0);
		}
	}
...
wlc_rsdb_opmode_change_cb(void *ctx, wlc_modesw_notif_cb_data_t *notif_data)
{
	...
	wlc_bsscfg_t *icfg = NULL, *join_pend_cfg = NULL, *ap_pend_cfg = NULL;
	...
	switch (notif_data->signal) {
		...
		case MODESW_UP_STA_COMPLETE:
		{
			...
			if (RSDB_ENAB(wlc->pub) &&
				...
				if (ap_pend_cfg) {
					wlc_rsdb_ap_bringup(wlc, &ap_pend_cfg);
					wlc_bsscfg_enable(ap_pend_cfg->wlc, ap_pend_cfg);
				}
			}
		}
		break;
...
wlc_rsdb_bmc_smac_template(void *hdl, int tplbuf, uint32 doublebufsize)
{
	 ...
	 */
	osh = (osl_t *)wlc->osh;
	if (wlc_rsdb_mode(pub->wlc) == PHYMODE_RSDB) {
