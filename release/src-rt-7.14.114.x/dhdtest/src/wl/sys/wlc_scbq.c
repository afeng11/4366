 ...
 *
 * $Id: wlc_scbq.c 526409 2015-01-14 06:12:12Z $
 */
...
wlc_scbq_module_attach(wlc_info_t *wlc)
{
	...
	wlc_txmod_fn_register(wlc->txmodi, TXMOD_SCBQ, scbq_info, scbq_txmod_fns);
...
#ifdef DONGLEBUILD
	qlen = (2 * scbq_info->wlc->pub->tunables->datahiwat) + PKTQ_LEN_DEFAULT +
	          scbq_info->wlc->pub->psq_pkts_total;
#else
	qlen = PKTQ_LEN_MAX;
#endif
	scbqlen = MIN(SCBQ_LEN_MAX, qlen/2);

	/* pktq_init() sets the length for the entire multi precendence queue */
	pktq_init(&scbq_cubby->scb_txq, WLC_PREC_COUNT, qlen);

	/* Set the per-precedence queue lengths */
	for (i = 0; i < WLC_PREC_COUNT; i++) {
		pktq_set_max_plen(&scbq_cubby->scb_txq, i, scbqlen);
	}

	/* Init tx path to our queue */
	wlc_txmod_config(scbq_info->wlc->txmodi, scb, TXMOD_SCBQ);
