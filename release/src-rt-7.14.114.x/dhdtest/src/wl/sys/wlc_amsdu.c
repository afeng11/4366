 ...
 *
 * $Id: wlc_amsdu.c 529218 2015-01-26 21:13:23Z $
 */
...
#if defined(WLAMSDU_TX) && (defined(BCMDBG) || defined(BCMDBG_AMSDU))
		wlc_amsdu_dump_scb,
#else
		NULL,
#endif
...
wlc_amsdu_up(void *hdl)
{
	...
	amsdu_info_t *ami = (amsdu_info_t *)hdl;
	int rxbufsz = ami->wlc->pub->tunables->rxbufsz;
	/* ensure tunable is a valid value which fits in a uint16 */
	ASSERT(rxbufsz > 0 && rxbufsz <= 0xffff);
	wlc_write_shm(ami->wlc, M_MAXRXFRM_LEN, (uint16)rxbufsz);
	return BCME_OK;
}
...
#ifdef WLAMSDU_TX
/** MSDU aggregation, per-A1-TID(priority) Return TRUE if packet consumed, otherwise FALSE */
static void
...
/** create a pseudo ether_header, the len field will be udpated when aggregating more frames */
static void *
 ...
 */
void * BCMFASTPATH
wlc_recvamsdu(amsdu_info_t *ami, wlc_d11rxhdr_t *wrxh, void *p, bool chained_sendup)
{
	osl_t *osh;
	amsdu_deagg_t *deagg;
	uint aggtype;
	...
	if (deagg->amsdu_deagg_state == WLC_AMSDU_DEAGG_LAST) {
		...
		/* if chained sendup, return back head pkt */
