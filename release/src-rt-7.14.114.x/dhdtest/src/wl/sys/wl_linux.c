 ...
 *
 * $Id: wl_linux.c 530342 2015-01-29 23:14:03Z $
 */
	...
	uint bustype, void *btparam, uint irq, uchar* bar1_addr, uint32 bar1_size, void *cmndata)
{
	...
	*/
	...
	if (!err && (dngl_agglimit & BCM_RPC_TP_AGG_SF_MASK) &&
		(dngl_agglimit & BCM_RPC_TP_AGG_BYTES_MASK)) {
		dbus_config_t config;
		config.config_id = DBUS_CONFIG_ID_AGGR_LIMIT;
		config.aggr_param.maxrxsf = dngl_agglimit >> BCM_RPC_TP_AGG_SF_SHIFT;
		config.aggr_param.maxrxsize = dngl_agglimit & BCM_RPC_TP_AGG_BYTES_MASK;
		config.aggr_param.maxtxsf = 0; /* not used yet */
		config.aggr_param.maxtxsize = 0; /* not used yet */
...
wl_monitor(wl_info_t *wl, wl_rxsts_t *rxsts, void *p)
{
	...
	if (wl->monitor_type == 1) {
		...
		strncpy(phdr->devname, wl->dev->name, sizeof(phdr->devname));
		phdr->devname[sizeof(phdr->devname) - 1] = '\0';
