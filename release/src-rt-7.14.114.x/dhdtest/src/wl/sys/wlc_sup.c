 ...
 *
 * $Id: wlc_sup.c 524513 2015-01-07 06:00:57Z $
 */
			...
			if (bcmwpa_is_wpa2_auth(wpa->WPA_auth)) {
				...
				if (wpa2ie) {
					...
					if (WLFBT_ENAB(sup->pub) &&
						...
						(BSS_FBT_INI_FBT(sup->wlc->fbt, sup->cfg)))
						wpa2ie_len =
							wpa2ie->len - sizeof(wpa_pmkid_list_t);
#endif
				}
	...
	if (pmk->flags & WSEC_PASSPHRASE) {
		...
		if (pmk->key_len == WSEC_MAX_PSK_LEN) {
			...
			if (wlc_wpa_cobble_pmk(info, (char *)pmk->key, pmk->key_len,
				...
				if (bcmwpa_includes_wpa2_auth(wpa->WPA_auth) && assoc) {
					int ret = BCME_OK;
#ifdef BCMSUP_PSK
					if (SUP_ENAB(wlc->pub) &&
	...
	struct ether_addr *auth_ea, uint8 *pmkid)
{
	...
	for (i = 0; i < sup->npmkid_sup; i++) {
		...
		if (!bcmp(auth_ea, &sup_pmkid->BSSID, ETHER_ADDR_LEN))
			break;
	}
	if (i == sup->npmkid_sup) {
		if (sup->npmkid_sup < SUP_MAXPMKID) {
			sup_pmkid = &sup->pmkid_sup[sup->npmkid_sup++];
		}
