 ...
 *
 * $Id: wlc_tpc.c 635239 2016-05-03 09:13:08Z $
 */
...
wlc_tpc_get_current(wlc_tpc_info_t *tpc, void *pwr, uint len, wlc_bsscfg_t *bsscfg)
{
...
#endif
#ifdef WL11ULB
	ASSERT(WL_BW_2P5MHZ == CLM_BW_2_5);
	ASSERT(WL_BW_5MHZ == CLM_BW_5);
	ASSERT(WL_BW_10MHZ == CLM_BW_10);
#endif /* WL11ULB */
	...
	if (old_power) {
		...
		uint8 txcore = 1;
		memset(tssisen_min, (uint8)WL_RATE_DISABLED, sizeof(tssisen_min));
...
#endif
		memset(pwr_to_wl->target_offsets, (uint8)WL_RATE_DISABLED,
		...
		if (!hwtable_txpwr)
		{
			...
			offset = wlc_stf_get_pwrperrate(wlc, pwr_to_wl->last_tx_ratespec,
				spatial_map) * 2;
