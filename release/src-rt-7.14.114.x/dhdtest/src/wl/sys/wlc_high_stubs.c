 ...
 *
 * $Id: wlc_high_stubs.c 530263 2015-01-29 18:21:14Z $
 */
	...
	case WLRPC_WLC_PHY_TXPOWER_SROMLIMIT_ID: {
		...
		if (!err) {
			...
			if ((max_pwr = ppr_create(wlc_hw->osh, ch_bw)) != NULL) {
				...
				wlc_phy_txpower_sromlimit(((wlc_hw_info_t *)wlc_hw)->band->pi,
					chan, &min_pwr, max_pwr, core);
...
#if defined(AP) && defined(RADAR)
		ret = phy_radar_detect_run((phy_info_t *)wlc_hw->band->pi);
#else
		ret = 0;
#endif
		err = bcm_xdr_pack_int32(&retb, ret);
...
#define HANDLE_BOUNDARY(b, p, prev_p)						\
	...
	case WLRPC_WLC_PHYCHAIN_ACTIVE_GET_ID : {
		...
		ret = (int8)wlc_phy_stf_duty_cycle_chain_active_get(wlc_hw->band->pi);
	...
	case WLRPC_WLC_PHY_INTERF_CHAN_STATS_UPDATE: {
		...
		wlc_phy_interf_chan_stats_update(((wlc_hw_info_t *)wlc_hw)->band->pi,
			(chanspec_t)chanspec, crsglitch, bphy_crsglitch, badplcp,
			bphy_badplcp, mbsstime);
		break;
	}
