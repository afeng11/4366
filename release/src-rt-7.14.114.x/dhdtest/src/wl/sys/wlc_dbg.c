...
wlc_print_per_pkt_cache_ac(wlc_info_t *wlc, d11actxh_t* acHdrPtr)
{
	...
	wlc_print_byte(" KeyIdx", cache_info->KeyIdx);
	wlc_print_byte(" PrimeMpduMax", cache_info->PrimeMpduMax);
	wlc_print_byte(" FallbackMpduMax", cache_info->FallbackMpduMax);
	wlc_print_word(" AmpduDur", ltoh16(cache_info->AmpduDur));
	wlc_print_byte(" BAWin", cache_info->BAWin);
	wlc_print_byte(" MaxAggLen", cache_info->MaxAggLen);
               ...
               wlc_txd_t *txd, wlc_d11rxhdr_t *wrxh, uint len)
{
	...
	defined(WLMSG_WSEC) || defined(WLEXTLOG) || defined(WLTEST)
int
