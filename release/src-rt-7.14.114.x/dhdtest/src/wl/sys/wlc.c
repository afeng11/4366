 ...
 *
 * $Id: wlc.c 635239 2016-05-03 09:13:08Z $
 */
...
#if defined(WLTEST)
#define MGMT_FR_EXP_MAX_TIME	300		/* Management Frame maximum expiry time */
#endif
...
#if defined(WLTEST)
static void wlc_mgmt_lifetime(wlc_cmn_info_t *cmn, int type, void *pkt);
#endif
...
enum {
	...
	IOV_HTFEATURES, /* Broadcom proprietary 11n rates. TODO: move to wlc_ht.c for trunk. */
#if defined(WLTEST)
	IOV_LIFETIME_MG,
#endif
...
static const bcm_iovar_t wlc_iovars[] = {
	...
	{"btc_select_profile", IOV_BTC_SELECT_PROFILE,
	(0), IOVT_BUFFER, 0
	},
	...
	(IOVF_OPEN_ALLOW), IOVT_BUFFER, sizeof(wl_cnt_info_t)
	},
	...
	(0), IOVT_BUFFER, sizeof(wl_lifetime_mg_t)
	},
#endif
#ifdef STA
...
#ifdef STA
	{"pspoll_prd", IOV_PSPOLL_PRD, IOVF_BSSCFG_STA_ONLY, IOVT_UINT16, 0},
...
static int _wlc_iovar_op(wlc_info_t *wlc, const char *name,
...
#ifdef STA
int wlc_send_action_frame_off_channel(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg,
...
#if defined(CCA_STATS) && !defined(DONGLEBUILD)
	int idx;
	wlc_bsscfg_t *cfg;
#endif /* CCA_STATS && !DONGLEBUILD */
	...
	*/
	...
	if (NBANDS(wlc) > 1) {
		...
		if (wlc->band->bandunit != bandunit || wlc->bandinit_pending) {
			...
			wlc_btcx_chspec_change_notify(wlc, chanspec, switchband);
#endif
	...
	actcb_fn_t act_cb, void *act_cb_arg)
{
	...
	if (!wlc->pub->up) {
		...
		if (!BSS_P2P_DISC_ENAB(wlc, cfg) && !BSS_AWDL_ENAB(wlc, cfg)) {
			/* override source MAC */
			sa_override = wlc_scanmac_get_mac(wlc->scan, macreq);
		}
...
#ifdef WL11N
	if (RSPEC_ISOFDM(lowest_basic_rspec)) {
...
wlc_macctrl_init(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	FOREACH_AS_STA(wlc, idx, bc) {
		if (BSS_PROXD_ENAB(wlc, bc) || BSSCFG_NAN(bc) || BSSCFG_AWDL(wlc, bc))
			ibss_bsscfgs--;
	}
#endif 
	...
	if ((cfg && cfg->BSS) || AP_ACTIVE(wlc) ||
		(wlc->stas_associated > 0 && ibss_bsscfgs == 0))
#else
	if ((cfg && cfg->BSS) || AP_ACTIVE(wlc) ||
...
BCMATTACHFN(wlc_attach_module_pre)(wlc_info_t *wlc)
{
	...
	MODULE_ATTACH(iocvi, wlc_iocv_high_attach, 2);
	...
	MODULE_ATTACH(hrti, wlc_hrt_attach, 5);
	MODULE_ATTACH(pcb, wlc_pcb_attach, 6);
	MODULE_ATTACH(bcmh, wlc_bsscfg_attach, 7);
	MODULE_ATTACH(scbstate, wlc_scb_attach, 8); /* scbstate before wlc_ap_attach() */
	MODULE_ATTACH(iemi, wlc_iem_attach, 9);
	MODULE_ATTACH(ieri, wlc_ier_attach, 10);
	MODULE_ATTACH(txmodi, wlc_txmod_attach, 11);
...
BCMATTACHFN(wlc_attach_module)(wlc_info_t *wlc)
{
	...
	MODULE_ATTACH(mchan, wlc_mchan_attach, 57);
#endif
	...
	MODULE_ATTACH(modesw, wlc_modesw_attach, 137);
#endif

#if defined(WL_PROT_OBSS) && !defined(WL_PROT_OBSS_DISABLED)
	MODULE_ATTACH(prot_obss, wlc_prot_obss_attach, 138);
#endif

#ifdef WL_PM_MUTE_TX
	MODULE_ATTACH(pm_mute_tx, wlc_pm_mute_tx_attach, 139);
#endif

#if defined(WL_BCNTRIM) && !defined(WL_BCNTRIM_DISABLED)
	MODULE_ATTACH(bcntrim, wlc_bcntrim_attach, 140);
#endif

#if defined(WL_RELMCAST) && !defined(WL_RELMCAST_DISABLED)
	MODULE_ATTACH(rmc, wlc_rmc_attach, 141 /* reliable multicast */);
#endif
	...
	{
	MODULE_ATTACH(pdsvc_info, wlc_proxd_attach, 142);
	}
#endif
	...
	if (wlc_macdbg_attach(wlc) != BCME_OK) {
		err = 146;
		goto fail;
	}
                      ...
                      osl_t *osh, void *regsva, uint bustype, void *btparam, void *objr, uint *perr)
{
	...
	*/
	...
	if ((pub->sih->boardtype == BCM943224X21) ||
		...
		wlc->stf->tempsense_disable = TRUE;
	           ...
	           ((pub->sih->boardtype == BCM94331X19) ||
	            ...
	            (pub->sih->boardtype == BCM943602X238D) ||
	...
	/* attach modules that must do first */
...
#endif
	MODULE_DETACH(eventq, wlc_eventq_detach);
...
#if defined(AP)
	MODULE_DETACH(ap, wlc_ap_detach);
#endif
...
#if defined(WL_MODESW) && !defined(WL_MODESW_DISABLED)
	MODULE_DETACH(modesw, wlc_modesw_detach);
#endif
...
#if defined(WL_BCNTRIM) && !defined(WL_BCNTRIM_DISABLED)
	MODULE_DETACH(bcntrim, wlc_bcntrim_detach);
#endif

#ifdef NEW_TXQ
	MODULE_DETACH(txqi, wlc_txq_detach);
#endif /* NEW_TXQ */
	wlc_macdbg_detach(wlc);
...
#if defined(GTKOE) && !defined(GTKOE_DISABLED)
	MODULE_DETACH(idsup, wlc_gtk_detach);
#endif /* defined(GTKOE) && !defined(GTKOE_DISABLED) */
	MODULE_DETACH(scan, wlc_scan_detach);
} /* wlc_detach_module */

/** detach modules that must do last after all other modules */
static void
...
wlc_watchdog(void *arg)
{
	...
	wlc_lq_noise_lte_update_ma(wlc,
		wlc_phy_noise_lte_avg((wlc_phy_t *)WLC_PI(wlc)));
#endif
...
_wlc_ioctl(void *ctx, int cmd, void *arg, int len, struct wlc_if *wlcif)
{
	...
	case WLC_SET_INFRA:
		...
		wlc_bsscfg_set_infra_mode(wlc, bsscfg, val ? TRUE : FALSE);
		break;
	...
	case WLC_GET_VAR: {
		...
		if (cmd == WLC_GET_VAR) {
			bcmerror = _wlc_iovar_op(wlc, arg, (int8*)arg + i, len - i, arg, len,
			                         IOVOP_FLAG_ALIGN_PARM, IOV_GET, wlcif);
		} else {
			bcmerror = _wlc_iovar_op(wlc, arg, NULL, 0, (int8*)arg + i, len - i,
...
#if defined(WL_EXPORT_CURPOWER)
	case WLC_CURRENT_PWR:
		bcmerror = wlc_iovar_op(wlc, "powertable", NULL, 0, arg, len, IOV_GET, wlcif);
		break;
#endif
...
wlc_iovar_op(wlc_info_t *wlc, const char *name,
	...
	bool set, struct wlc_if *wlcif)
{
	return _wlc_iovar_op(wlc, name, params, p_len, arg, len, 0, set, wlcif);
}
 ...
 */
static int
_wlc_iovar_op(wlc_info_t *wlc, const char *name,
	void *params, int p_len, void *arg, int len, uint flags,
	bool set, struct wlc_if *wlcif)
{
	int err = BCME_OK;
...
doiovar:
	...
	if (IOVOP_ALIGN_PARM(flags)) {
		...
		if (set) {
			/* caller params:
			 * _wlc_iovar_op(wlc, arg, NULL, 0, (int8*)arg + i, len - i,
			 *               IOVOP_FLAG_ALIGN_PARM, IOV_SET, wlcif);
			 */
			...
			/* caller params:
			 * _wlc_iovar_op(wlc, arg, (int8*)arg + i, len - i, arg, len,
			 *               IOVOP_FLAG_ALIGN_PARM, IOV_GET, wlcif);
			 */
...
#if defined(WLRSDB)
		if (RSDB_ENAB(wlc->pub) &&
		    wlc_rsdb_chkiovar(vi, actionid, wlc_idx)) {
	...
	void *params, uint p_len, void *arg, int len, int val_size, struct wlc_if *wlcif)
{
	...
	*/
	...
	{
		wlc_cmn_info_t *cmn = wlc->cmn;
		memcpy(arg, cmn->lifetime_mg, sizeof(wl_lifetime_mg_t));
		if (cmn->lifetime_mg->lifetime <= MGMT_FR_EXP_MAX_TIME) {
			 cmn->lifetime_mg->lifetime = cmn->lifetime_mg->lifetime*1000; /* ms ->us */
		} else {
			err = BCME_RANGE;
		}
		break;
	...
	int paramlen, void *bufptr, int buflen, int *perr)
{
	...
	iolen = WLC_IOVBUF_BSSCFGIDX_PREFIX_SIZE + namelen + sizeof(int32) + paramlen;
	...
	/* copy prefix, no null */
	bcopy(WLC_IOVBUF_BSSCFGIDX_PREFIX, p, WLC_IOVBUF_BSSCFGIDX_PREFIX_SIZE);
	p += WLC_IOVBUF_BSSCFGIDX_PREFIX_SIZE;
	...
	bool need_ies)
{
	...
	if (bi->flags2 & WLC_BSS_VHT) {
		...
		wl_bi.vht_txmcsmap_prop = bi->rateset.vht_mcsmap_prop;
	}
#endif /* WL11AC */
	/* ie length */
...
wlc_dump_wlc(wlc_info_t *wlc, struct bcmstrbuf *b)
{
	...
	for (qi = wlc->tx_queues; qi != NULL; qi = qi->next) {
		bcm_bprintf(b, "txqinfo %p len %d stopped 0x%x\n",
		            qi, pktq_len(WLC_GET_TXQ(qi)), qi->stopped);
...
#ifdef VASIP_HW_SUPPORT
#ifdef WLC_LOW
	bcm_bprintf(b, "\tvasipcodever %d.%d\n", wlc->vasip_major, wlc->vasip_minor);
#endif /* WLC_LOW */
#endif /* VASIP_HW_SUPPORT */
...
wlc_dump_wme(wlc_info_t *wlc, struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b, "xfifordy 0x%x psm_b0 0x%x txf_cur_idx 0x%x\n",
		R_REG(osh, &regs->u.d11regs.xmtfifordy),
...
wlc_dump_stats(wlc_info_t *wlc, struct bcmstrbuf * b)
{
	...
	for (i = 0; i < NREINITREASONCOUNT; i++) {
		bcm_bprintf(b, "%d(%d) ", i, cnt->reinitreason[i]);
	}
	PRNL();
	...
	PRVAL(txchanrej); PRVAL(tbtt); PRNL();
...
wlc_statsupd(wlc_info_t *wlc)
{
	...
	wlc_bmac_copyfrom_shm(wlc->hw, M_UCODE_MACSTAT, macstat_readbuf,
...
wlc_ctrupd(wlc_info_t *wlc, uint macstat_offset)
{
	...
	v = wlc_bmac_read_shm(wlc->hw, MACSTAT_ADDR(macstat_offset));
...
/** called by e.g. 'NIC + offloads' functionality */
uint16
wlc_acphy_txctl1_calc_ex(wlc_info_t *wlc, ratespec_t rspec, int8 lpc_offset,
	uint8 txpwr, bool is_mu)
{
	return wlc_acphy_txctl1_calc(wlc, rspec, lpc_offset, txpwr, is_mu);
}
...
/** called by e.g. 'NIC + offloads' functionality */
uint16 wlc_acphy_txctl2_calc_ex(wlc_info_t *wlc, ratespec_t rspec, uint8 txbf_uidx)
{
	return wlc_acphy_txctl2_calc(wlc, rspec, txbf_uidx);
}
 ...
 * Returns TRUE if the packet should be wlc_suppressed.
 */
bool
...
/** PROP_TXSTATUS specific function */
void
wlc_process_wlhdr_txstatus(wlc_info_t *wlc, uint8 status_flag, void* p, bool hold)
{
	wlfc_process_wlhdr_complete_txstatus(wlc->wl, status_flag, p, NULL, hold);
}

#ifdef WLFCTS
/** PROP_TXSTATUS specific function */
void
wlc_process_wlfc_dbg_update(wlc_info_t *wlc, uint8 dbg_type, void* p)
{
	...
	wlfc_push_signal_data(wlc->wl, results, sizeof(results), TRUE);
}
#endif /* WLFCTS */
...
wlc_dotxstatus(wlc_info_t *wlc, tx_status_t *txs, uint32 frm_tx2)
{
...
#if defined(WLAMPDU) && defined(BCMPCIEDEV) && defined(PROP_TXSTATUS)
	/* in pcie FD proptxstatus overwrites pktdata in txstatus path
	 * so in case ampdu chain has to be freed don't corrupt the data here
	 * as it will be revisited in wlc_ampdu_free_chain called from wlc_ampdu_dotxstatus
	 */
	if (BCMPCIEDEV_ENAB() && PROP_TXSTATUS_ENAB(wlc->pub))
		ampdu_free_chain = wlc_ampdu_if_freechain(wlc->ampdu_tx, p, scb);
#endif /* WLAMPDU && BCMPCIEDEV && PROP_TXSTATUS */

#ifdef PROP_TXSTATUS
	from_host = WL_TXSTATUS_GET_FLAGS(pkttag->wl_hdr_information) & WLFC_PKTFLAG_PKTFROMHOST;
	if (PROP_TXSTATUS_ENAB(wlc->pub)) {
		if (!(scb && SCB_ISMULTI(scb)) && (!ampdu_free_chain)) {
			if (scb &&
			    (WLFC_D11_STATUS_INTERPRET(txs) == WLFC_CTL_PKTFLAG_D11SUPPRESS)) {
#ifdef WLAMPDU
				...
				wlc_process_wlhdr_txstatus(wlc, WLFC_D11_STATUS_INTERPRET(txs),
				                           p, FALSE);
			}
		}
	}
	...
	if (txs->status.was_acked) {
		...
		if ((fc == FC_AUTH) || (fc == FC_ASSOC_REQ))
			wlc_assoc_auth_txstatus(wlc, bsscfg, fc);
#endif
...
#else
	wlc_write_shm(wlc, M_DOT11_DTIMPERIOD, dtim);
#endif /* WLMCNX */
...
#else
	*dtim = wlc_read_shm(wlc, M_DOT11_DTIMPERIOD);
#endif /* WLMCNX */
...
wlc_set_pmstate(wlc_bsscfg_t *cfg, bool state)
{
	 ...
	 */
	/* Timely delivery of NULL PKTs to indicate PS is essential.
	 * In some cases the NULL PKTs are are getting sent out to air
	 * very late due to medium being extremely busy. Thereby
	 * increasing PRIO to MAX in order to ensure NULL PKTs
	 * get sent out to air in a more timely manner
	 */

	else if (wlc->excursion_active ||
	         !wlc_sendpmnotif(wlc, cfg, &cfg->BSSID, 0, MAXPRIO,
...
{
	return (WLC_BITSCNT(wlc->stf->hw_rxchain) > 1) && wlc_stf_rxchain_ishwdef(wlc);
}
...
wlc_write_rate_shm(wlc_info_t *wlc, uint8 rate, uint8 basic_rate)
{
	...
	basic_table = RATE_ISOFDM(rate) ? M_RT_BBRSMAP_A : M_RT_BBRSMAP_B;
...
{
	int lifetime = 0;
	wlc_info_t *wlc = (wlc_info_t*)cmn->wlc;
	/* Set packet lifetime if configured */
	if (cmn->lifetime_mg->mgmt_bitmap & type)
		lifetime = cmn->lifetime_mg->lifetime;
	if (!(WLPKTTAG(pkt)->flags & WLF_EXPTIME) && (lifetime > 0))
		wlc_lifetime_set(wlc, pkt, lifetime);
}
#endif
...
)
{
	...
	ASSERT(qi != NULL);
#if defined(WLTEST)
	wlc_mgmt_lifetime(wlc->cmn, MGMT_AUTH_LT, pkt);
#endif
...
#endif
#if defined(WLTEST)
	wlc_mgmt_lifetime(wlc->cmn, MGMT_ASSOC_LT, pkt);
#endif
...
} /* wlc_bss_eapol_wai_event_indicate */
...
#ifdef WLCHANIM
						chamin_stats = wlc_lq_chanim_chanspec_to_stats(
							wlc->chanim_info,
							wlc->home_chanspec);
...
wlc_get_pmax_tid(wlc_info_t *wlc, uint tid)
{
	...
	FOREACHSCB(wlc->scbstate, &scbiter, scb) {
		pmax += pktq_pmax(wlc_scbq_txq(wlc->tx_scbq, scb), tid);
	}
#else
	if (wlc_ampdu_tid_enabled(wlc, tid)) {
		...
		wlc_txq_info_t *qi = wlc->active_queue;
		pmax += pktq_pmax(WLC_GET_TXQ(qi), WLC_PRIO_TO_PREC(tid));
	}
#endif
	return pmax;
}
...
wlc_get_prec_txpending(wlc_info_t *wlc, int tid)
{
	...
	/* Count packets in low txq */
	pktcnt += pktq_plen(WLC_GET_TXQ(qi), prec);
	pktcnt += pktq_plen(WLC_GET_TXQ(qi), prec+1);

#ifdef TXQ_MUX
	/* Count all packets in the SCB queues */
	FOREACHSCB(wlc->scbstate, &scbiter, scb) {
		pktcnt += pktq_plen(wlc_scbq_txq(wlc->tx_scbq, scb), tid);
	}
#else
...
void wlc_check_txq_fc(wlc_info_t *wlc, wlc_txq_info_t *qi)
{
	if (!EDCF_ENAB(wlc->pub) || (wlc->pub->wlfeatureflag & WL_SWFL_FLOWCONTROL)) {
		if (wlc_txflowcontrol_prio_isset(wlc, qi, ALLPRIO) &&
		    (pktq_len(WLC_GET_TXQ(qi)) < wlc->pub->tunables->datahiwat / 2)) {
		...
		for (prio = MAXPRIO; prio >= 0; prio--) {
			if (wlc_txflowcontrol_prio_isset(wlc, qi, prio) &&
			    (pktq_plen(WLC_GET_TXQ(qi), wlc_prio2prec_map[prio]) <
...
#if defined(WLC_HIGH) && defined(WL_MULTIQUEUE)
				wlc_bmac_enable_tx_hostmem_access(wlc_iter->hw,
					wlc_iter->cmn->hostmem_access_enabled);
#endif /* WLC_HIGH && WL_MULTIQUEUE */
			}
		}
	}
#endif /* BCMPCIEDEV */
...
int
wlc_dump_register_wrapper(wlc_info_t *wlc, const char *name, dump_fn_t dump_fn, void *dump_fn_arg)
{
	return wlc_dump_register(wlc->pub, name, dump_fn, dump_fn_arg);
}

int
wlc_get_wme(wlc_info_t *wlc)
{
	ASSERT(wlc && wlc->pub);
	return wlc->pub->_wme;
}
...
void
wlc_cts_send(wlc_info_t *wlc, uint16 duration)
{
	int mac_depth = 0;
	int orig_state;
	...
	wlc_write_shm(wlc, M_CTS_DURATION, duration);
