...
#define SHM_MBSS_BASE_ADDR	(D11REV_GE(wlc->pub->corerev, 40) ? \
				 SHM_MBSS_UCODE_AC_BASE : SHM_MBSS_UCODE_BASE)
...
#define SHM_MBSS_SSID_ADDR(idx)	((idx) == 0 ? M_SSID : SHM_MBSS_WORD_OFFSET_TO_ADDR(0x10 * (idx)))
...
wlc_mbss_up(void *hdl)
{
	...
	if (D11REV_ISMBSS4(wlc->pub->corerev)) {
		...
		for (idx = 0; idx < wlc->pub->tunables->maxucodebss4; idx++) {
			wlc_write_shm(wlc, offset, INVALIDFID);
			offset += 2;
		}
	}
#ifdef WLLPRS
...
{
	wlc->mbss->last_tbtt_us = R_REG(wlc->osh, &wlc->regs->tsf_timerlow);
}
