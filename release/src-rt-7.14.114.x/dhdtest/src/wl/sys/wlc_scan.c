 ...
 *
 * $Id: wlc_scan.c 530263 2015-01-29 18:21:14Z $
 */
	...
	int nprobes, bool split_scan, int nssid, wlc_ssid_t *ssid, scancb_fn_t fn, void *arg)
{
	...
	/* override source MAC */
	sa_override = wlc_scanmac_get_mac(wlc->scan, WLC_ACTION_SCAN);
	...
	if (enable) {
		...
		if ((idx = wlc_bsscfg_get_free_idx(wlc)) == -1) {
			...
			return BCME_NORESOURCE;
		                ...
		                &scan_info->scanmac_config.mac, FALSE)) == NULL) {
...
scanmac_set(scan_info_t *scan_info, void *arg, int len)
{
	...
	/* verify length */
	if (len < OFFSETOF(wl_scanmac_t, data) ||
...
wlc_bsscfg_t *
wlc_scanmac_get_bsscfg(wlc_scan_info_t *scan, int macreq)
{
	...
	if (scan_info->scanmac_bsscfg != NULL) {
		...
		bool is_associated = IS_ASSOCIATED(scan_info);
...
struct ether_addr *
wlc_scanmac_get_mac(wlc_scan_info_t *scan, int macreq)
{
	wlc_bsscfg_t *bsscfg = wlc_scanmac_get_bsscfg(scan, macreq);
...
wlc_scanmac_update(wlc_scan_info_t *scan)
{
	...
	return scanmac_update_mac(scan_info);
}
