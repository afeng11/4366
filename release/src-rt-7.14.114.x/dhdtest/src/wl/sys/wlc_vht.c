 ...
 *
 * $Id: wlc_vht.c 528293 2015-01-21 23:18:59Z $
 */
...
void
wlc_vht_upd_txbf_cap(wlc_vht_info_t *vhti, uint8 bfr, uint8 bfe)
{
...
#ifdef WL_PROT_OBSS
	if (WLC_PROT_OBSS_ENAB(cfg->wlc->pub)) {
...
#ifdef WL_MODESW
void wlc_vht_sync_bcn_opmode(wlc_vht_info_t *vhti, wlc_bsscfg_t *bsscfg,
	struct scb *scb, chanspec_t max_opermode_chanspec)
{
	wlc_info_t *wlc;
	uint8 ap_oper_mode;
	uint8 ap_rxnss;
	...
	ap_rxnss = DOT11_OPER_MODE_RXNSS(
			wlc_vht_get_scb_opermode(vhti, scb));

	/* Overwrite the original chanspec for future
	 * use during upgrade
	 */
	wlc_modesw_set_max_chanspec(wlc->modesw, bsscfg,
		max_opermode_chanspec);

	if (wlc_vht_get_scb_opermode_enab(vhti, scb)) {
		ap_oper_mode = wlc_modesw_derive_opermode(
			wlc->modesw, max_opermode_chanspec,
			bsscfg, ap_rxnss);
	}
	else {
		ap_oper_mode = wlc_modesw_derive_opermode(
			wlc->modesw, max_opermode_chanspec,
			bsscfg, wlc->stf->rxstreams);
	}

	/* Perform ratesel init to allow new rate spec to be used
	 * based upon AP's operating mode
	 */
	wlc_vht_update_scb_oper_mode(vhti, scb, ap_oper_mode);
	return;
}
#endif /* WL_MODESW */
...
wlc_vht_up(void *ctx)
{
	...
	if (WLC_VHT_FEATURES_GET(wlc->pub, WL_VHT_FEATURES_1024QAM)) {
		wlc_rateset_prop_vhtmcs_build(&bss->rateset, wlc->stf->op_rxstreams);
	} else {
		bss->rateset.vht_mcsmap_prop = VHT_PROP_MCS_MAP_NONE_ALL;
	}
	mcsmap_prop = wlc_rateset_filter_mcsmap(bss->rateset.vht_mcsmap_prop,
		wlc->stf->txstreams, (WLC_VHT_FEATURES_PROP_MCS_GET(wlc->pub)));
	...
	bss->rateset.vht_mcsmap_prop = mcsmap_prop;
