 ...
 *
 * $Id: wlc_rrm.c 523117 2014-12-26 18:32:49Z $
 */
...
static void wlc_rrm_add_bcnnbr(wlc_rrm_info_t *rrm_info, struct scb *scb,
...
static void wlc_rrm_scb_state_upd(bcm_notif_client_data ctx, bcm_notif_server_data scb_data);
...
BCMATTACHFN(wlc_rrm_attach)(wlc_info_t *wlc)
{
	...
	if (wlc_scb_state_upd_register(wlc,
		wlc_rrm_scb_state_upd, (void*)rrm_info) != BCME_OK) {
...
#ifdef WL11K_AP
	wlc_scb_state_upd_unregister(wlc, wlc_rrm_scb_state_upd, (void*)rrm_info);
#endif /* WL11K_AP */
	...
	for (; ie != NULL && idx < count;
		...
		case DOT11_MEASURE_TYPE_FTMRANGE: {
			...
			if (!PROXD_ENAB(wlc->pub))
				break;
			WL_ERROR(("wl%d: %s: FTM Range request, not yet supported\n",
	...
	for (; ie != NULL && idx < count;
		...
		switch (ie->type) {
			...
			case DOT11_MEASURE_TYPE_FTMRANGE: {
				...
				if (!PROXD_ENAB(wlc->pub))
					break;
				WL_ERROR(("wl%d: %s: FTM Range response, not yet supported\n",
					WLCWLUNIT(wlc), __FUNCTION__));
				break;
			}
#endif /* WL_FTM */
	...
	rrm_bsscfg_cubby_t *rrm_cfg)
{
	...
	if (nbr_rep) {
		...
		memcpy(&nbr_rep->nbr_elt, nbr_elt, sizeof(nbr_rpt_elem_t));
	...
	struct dot11_bcn_prb *bcn, bcm_tlv_t *htie, bcm_tlv_t *mdie, uint32 flags)
{
	...
	if (nbr_rep) {
		...
		if (nbr_rep_cnt >= MAX_BEACON_NEIGHBORS) {
			WL_ERROR(("%s over MAX_BEACON_NEIGHBORS\n", __FUNCTION__));
			return;
		}
		nbr_rep = (rrm_nbr_rep_t *)MALLOCZ(rrm_info->wlc->osh, sizeof(rrm_nbr_rep_t));
		if (nbr_rep == NULL) {
