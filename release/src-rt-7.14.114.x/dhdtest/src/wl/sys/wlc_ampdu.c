 ...
 *
 * $Id: wlc_ampdu.c 530401 2015-01-30 00:33:38Z $
 */
...
typedef struct scb_ampdu_tx {
	...
	uint8 max_rxlen_factor; /**< max ampdu_rcv len specified in power of 2 */
...
/* local prototypes */
void wlc_ampdu_tx_cfg_init(wlc_info_t *wlc, ampdu_tx_config_t *ampdu_tx_cfg);
/* scb cubby */
...
fail:
	if (ampdu_tx->config && obj_registry_unref(wlc->objr, OBJR_AMPDUTX_CONFIG) == 0) {
...
wlc_ampdu_dbg_stats(wlc_info_t *wlc, struct scb *scb, scb_ampdu_tid_ini_t *ini)
{
	...
	now_txallfrm = wlc_read_shm(wlc, MACSTAT_ADDR(MCSTOFF_TXFRAME));
	now_txbcn = wlc_read_shm(wlc, MACSTAT_ADDR(MCSTOFF_TXBCNFRM));
	now_txrts = wlc_read_shm(wlc, MACSTAT_ADDR(MCSTOFF_TXRTSFRM));
	now_rxcts = wlc_read_shm(wlc, MACSTAT_ADDR(MCSTOFF_RXCTSUCAST));
	now_rsptmout = wlc_read_shm(wlc, MACSTAT_ADDR(MCSTOFF_RXRSPTMOUT));
	now_rxstrt = wlc_read_shm(wlc, MACSTAT_ADDR(MCSTOFF_RXSTRT));
...
/**
 * Called after e.g. a higher software layer gave this AMPDU layer an MSDU, and the AMPDU layer put
 * the MSDU on an AMPDU internal software queue.
 */
INLINE static void
...
/**
 * Function which contains the smarts of when to 'release' (wlc_ampdu_release) agreggated sdu's
 * (that, at this stage, have not been sent yet) from an AMPDU internal software queue towards the
 * wlc transmit queue, bringing them closer to the d11 core. This function is called from various
 * places.
 *
 * Parameters:
 *    ini: connection with a specific remote party and a specific TID/QoS category.
 *
	...
	while (p) {
		...
		if (AMPDU_MAC_ENAB(wlc->pub)) {
			...
			if (AMPDU_HW_ENAB(wlc->pub)) {
				...
				txh->u2.s1.MaxAggBytes = scb_ampdu->max_rxlen_factor;
			} else {
...
bool wlc_ampdu_if_freechain(ampdu_tx_info_t *ampdu_tx, void *p, struct scb *scb)
{
	if (!scb || !SCB_AMPDU(scb))
		return TRUE;
                                  ...
                                  void *p, tx_status_t *txs, wlc_txh_info_t *txh_info)
{
	...
	*/
	...
	defined(BCMDBG_AMPDU))
		...
		*/
		...
		if (p && PROP_TXSTATUS_ENAB(wlc->pub)) {
			uint8 wlfc_status =  wlc_wlfc_d11_txstatus_interpret(&txs->status);
			if (!acked && scb && (wlfc_status == WLFC_CTL_PKTFLAG_D11SUPPRESS)) {
				...
				wlfc_process_wlhdr_complete_txstatus(wlc->wl, wlfc_status, p,
	...
	if (fix_rate) {
		...
		WLCNTSCBADD(scb->scb_stats.tx_pkts_total, txsucc_cnt[0]);
	...
	if (update_rate) {
		...
		if (tx_cnt[0] == 0 && fbr_cnt == 0) {
			...
			wlc_scb_ratesel_upd_txs_ampdu(wlc->wrsi, scb, txs->frameid,
				(uint)tx_cnt[0], 0, 0, 0, FALSE, mcs[0], is_sgi[0], antselid);
#ifdef PSPRETEND
			if (pps_retry) {
...
#endif /* PSPRETEND */
			wlc_scb_ratesel_upd_txs_ampdu(wlc->wrsi, scb, txs->frameid,
				(uint)tx_cnt[0], (uint)txsucc_cnt[0], (uint)tx_cnt[1],
				(uint)txsucc_cnt[1], FALSE, mcs[0], is_sgi[0], antselid);
		}
	...
	uint32 s1, uint32 s2, uint32 s3, uint32 s4)
{
	...
	if (AMPDU_HOST_ENAB(wlc->pub)) {
		...
		WLCNTSCBADD(scb->scb_stats.tx_pkts_total, txsucc_mrt + txsucc_fbr);
					...
					if (PLCP3_ISSGI(txh->FragPLCPFallback[3])) {
						...
						if (ampdu_tx->amdbg)
							WLCNTADD(ampdu_tx->amdbg->txmcssgi
								[MCS2IDX(mcs_fb)], tx_fbr);
					}
					if (PLCP3_ISSTBC(txh->FragPLCPFallback[3])) {
						WLCNTADD(ampdu_tx->cnt->u2.txmpdu_stbc, tx_fbr);
						if (ampdu_tx->amdbg)
						   WLCNTADD(ampdu_tx->amdbg->txmcsstbc[mcs_fb],
						            tx_fbr);
					}
				}
#endif /* BCMDBG || WLTEST */
...
nextp:
		...
		tot_mpdu++;
				...
				if (p && PROP_TXSTATUS_ENAB(wlc->pub)) {
					uint8 wlfc_status =
						wlc_wlfc_d11_txstatus_interpret(&txs->status);
					if (!ack_recd && scb &&
	...
	if (AMPDU_MAC_ENAB(wlc->pub)) {
		if (tx_mrt + tx_fbr == 0) {
			...
			if (update_rate)
				...
				(uint)ncons, 0, 0, 0, FALSE, mcs, is_sgi, antselid);
		} else if (update_rate)
			wlc_scb_ratesel_upd_txs_ampdu(wlc->wrsi, scb, first_frameid,
				tx_mrt, txsucc_mrt, tx_fbr, txsucc_fbr,
				FALSE, mcs, is_sgi, antselid);
	} else
#endif /* WLAMPDU_MAC */
	if (update_rate) {
	...
	bool override)
{
	...
	scb_ampdu->max_rxlen_factor = max_rxlen_factor;
...
wlc_ampdu_tx_dump(ampdu_tx_info_t *ampdu_tx, struct bcmstrbuf *b)
{
	...
	for (i = 0, max_val = 0, last = 0; i <= AMPDU_HT_MCS_LAST_EL; i++) {
		...
		if (cnt->txmpdu_cnt[i])
			last = i; /* highest used MCS */
	}
	/* round up to highest MCS with same number of spatial streams */
	last = 8 * (last / 8 + 1) - 1;
	if (max_val) {
		bcm_bprintf(b, "PER : ");
		for (i = 0; i <= last; i++) {
...
#ifdef WLATF
				bcm_bprintf(b, "\tbytes_in_flight %d\n",
					ini->released_bytes_inflight);
#endif
...
wlc_ampdu_txrates_get(ampdu_tx_info_t *ampdu_tx, wifi_rate_stat_t *rate, int i, bool vht)
{
	if (vht) {
		...
		if (i < AMPDU_HT_MCS_ARRAY_SIZE) {
			rate->tx_mpdu = ampdu_tx->amdbg->txmcs[i];
			rate->mpdu_lost = rate->retries = rate->tx_mpdu -
				ampdu_tx->amdbg->txmcs_succ[i];
		}
	}
