 ...
 *
 * $Id: wlc_apps.c 527938 2015-01-20 19:43:17Z $
 */
...
/* PS transition status flags */
#define PS_TRANS_OFF_PEND	0x01	/* Pend PS off until txfifo draining is done */
#define PS_TRANS_OFF_BLOCKED	0x02	/* Block attempts to switch PS off */
#define PS_TRANS_OFF_IN_PROG	0x04	/* PS off transition is already in progress */
...
wlc_apps_scb_ps_off(wlc_info_t *wlc, struct scb *scb, bool discard)
{
	...
	WL_PS(("wl%d.%d: wlc_apps_scb_ps_off, %s aid %d\n", wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg),
	 ...
	 * just return.
	 */
	if ((scb_psinfo->ps_trans_status & PS_TRANS_OFF_IN_PROG))
		return;
	scb_psinfo->ps_trans_status |= PS_TRANS_OFF_IN_PROG;
	...
	if (!ETHER_ISMULTI(&ea) && (wlc_scbfind(wlc, bsscfg, &ea) == NULL)) {
		WL_PS(("wl%d: %s: exiting, scb for %s was freed",
...
wlc_apps_scb_ps_on(wlc_info_t *wlc, struct scb *scb)
{
	...
	WL_PS(("wl%d.%d wlc_apps_scb_ps_on, %s aid %d, pkts pending %d\n", wlc->pub->unit,
	...
	scb->PS = TRUE;
	scb->tbtt = 0;
	scb_psinfo->ps_trans_status &= ~PS_TRANS_OFF_PEND;
...
#if defined(BCMDBG) || defined(WLMSG_PS)
		if (MBSS_ENAB(wlc->pub)) {
			uint32 mc_fifo_pkts = wlc_mbss_get_bcmc_pkts_sent(wlc, scb->bsscfg);

			if (mc_fifo_pkts != 0) {
				WL_PS(("wl%d.%d: START PS-ON; bcmc %d\n", wlc->pub->unit,
					WLC_BSSCFG_IDX(scb->bsscfg), mc_fifo_pkts));
			}
		}
	...
	if ((wlc->block_datafifo & DATA_BLOCK_PS) && txpktpendtot == 0) {
		...
		FOREACHSCB(wlc->scbstate, &scbiter, scb) {
			...
			if (SCB_PS(scb) && !SCB_ISMULTI(scb)) {
				...
				if ((scb_psinfo->ps_trans_status & PS_TRANS_OFF_PEND)) {
#ifdef PSPRETEND
...
wlc_apps_psq_ageing_needed(wlc_info_t *wlc, struct scb *scb)
{
	...
	if (WLWNM_ENAB(wlc->pub)) {
		...
		if (SCB_WNM_SLEEP(wnm_scbcap) && sleep_interval) {
			...
			interval = MAX(scb->listen, interval);

			return (scb->tbtt >= interval);
		}
		else
			return (scb->tbtt >= scb->listen);
	}
	else
#endif /* WLWNM_AP */
		return (scb->tbtt >= scb->listen);
	...
	*/
	if (ps_count == 0 && PROP_TXSTATUS_ENAB(wlc->pub) &&
		...
		if (scb->apsd.ac_delv == AC_BITMAP_NONE ||
			scb->apsd.ac_delv == AC_BITMAP_ALL) {
			/* if no ACs are delivery-enabled (legacy), or all ACs are
			 * delivery-enabled (special case), the PVB should
			 * indicate if any packet is buffered.
			 */
			ps_count = (SCB_PROPTXTSTATUS_TIM(scb) ? 1 : 0);
		}
		else {
			/* Otherwise, the PVB should indicate if any packets are buffered
			 * for non-delivery-enabled ACs only
			 */
			ps_count = ((SCB_PROPTXTSTATUS_TIM(scb) &
				(~scb->apsd.ac_delv & AC_BITMAP_ALL)) &&
				(bss_info->ps_nodes > 0) ? 1 : 0);
		}
	}
#endif /* PROP_TXSTATUS */
...
wlc_apps_process_ps_switch(wlc_info_t *wlc, struct ether_addr *ea, int8 ps_on)
{
	...
	if (ps_on) {
		if ((ps_on & PS_SWITCH_PMQ_SUPPR_PKT)) {
			WL_PS(("%s %d: Req by suppr pkt!\n", __FUNCTION__, __LINE__));
			scb_psinfo->ps_trans_status |= PS_TRANS_OFF_BLOCKED;
		} else {
			WL_PS(("%s %d: Actual PMQ entry addition!\n", __FUNCTION__, __LINE__));
	...
	} else if ((scb_psinfo->ps_trans_status & PS_TRANS_OFF_BLOCKED)) {
...
wlc_apps_suppr_frame_enq(wlc_info_t *wlc, void *pkt, tx_status_t *txs, bool last_frag)
{
	...
	if (!SCB_PS(scb)) {
		 ...
		 */
		WL_PS(("%s %d: PMQ entry interrupt delayed!\n", __FUNCTION__, __LINE__));
