 ...
 *
 * $Id: wlc_bsscfg.c 528553 2015-01-22 14:24:42Z $
 */
	...
	void *params, uint p_len, void *arg, int len, int val_size, struct wlc_if *wlcif)
{
	...
	case IOV_GVAL(IOV_BSSCFG_INFO): {
		wlc_bsscfg_info_t *bc_info = arg;

		bc_info->type = bsscfg->type;
		if (bc_info->type == BSSCFG_TYPE_GENERIC) {
			if (BSSCFG_AP(bsscfg)) {
				bc_info->subtype = BSSCFG_GENERIC_AP;
			} else {
				bc_info->subtype = BSSCFG_GENERIC_STA;
			}
		}
		if (bc_info->type == BSSCFG_TYPE_P2P) {
			if (BSS_P2P_DISC_ENAB(wlc, bsscfg)) {
				bc_info->subtype = BSSCFG_P2P_DISC;
			}
			if (P2P_GO(wlc, bsscfg)) {
				bc_info->subtype = BSSCFG_P2P_GO;
			}
			if (P2P_CLIENT(wlc, bsscfg)) {
				bc_info->subtype = BSSCFG_P2P_GC;
			}
		}
		break;
	}
...
wlc_bsscfg_malloc(wlc_info_t *wlc)
{
	...
	wlc_bsscfg_t *cfg;
#ifdef WL_FTM
	STATIC_ASSERT(sizeof(cfg->ext_cap) == sizeof(uint8 *));
#endif /* WL_FTM */
...
#ifdef WL_FTM
	cfg->ext_cap = MALLOCZ(osh, DOT11_EXTCAP_LEN_MAX);
	if (!cfg->ext_cap) {
		goto fail;
	}
#endif
...
#ifdef WL_FTM
	if (cfg->ext_cap)
		MFREE(osh, cfg->ext_cap, DOT11_EXTCAP_LEN_MAX);
#endif /* WL_FTM */
...
wlc_bsscfg_free(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg)
{
	...
	if (!(bsscfg->flags & WLC_BSSCFG_P2P_RECREATE_BSSIDX)) {
		...
		if (bsscfg != primary && bsscfg->wlcif != NULL) {
