 ...
 *
 * $Id: wlc_txbf.c 530263 2015-01-29 18:21:14Z $
 */
...
wlc_txbf_up(void *context)
{
	 ...
	 * use fixed index 5
	*/
	val = wlc_read_shm(wlc, (txbf->shm_base + 5 * M_BFI_BLK_SIZE  + C_BFI_BFRIDX_POS) * 2);
	val &= (~(1 << 8));
	wlc_write_shm(wlc, (txbf->shm_base +  5 * M_BFI_BLK_SIZE + C_BFI_BFRIDX_POS) * 2, val);
	...
	txpwr204080_t* txpwrs)
{
	...
	*shm_index = txbf_scb_info->shm_index;
	...
	if (RSPEC_ISCCK(rspec) || (txbf->mode == 0))
		return FALSE;
...
static void wlc_txbf_shm_dump(wlc_txbf_info_t *txbf, struct bcmstrbuf *b)
{
	...
	for (i = 0; i < AMT_SIZE; i++) {
		val = wlc_read_amtinfo_by_idx(wlc, i);
		if (val)
