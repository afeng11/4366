 ...
 *
 * $Id: wlc_ap.c 528366 2015-01-22 04:13:27Z $
 */
		...
		if (BSSCFG_AP(bsscfg) && bsscfg->up) {
			if (MBSS_BCN_ENAB(wlc, bsscfg)) {
				...
				if (IOV_ID(actionid) == IOV_BEACON_INFO) {
					pkt = wlc_mbss_get_bcn_template(wlc, bsscfg);
				} else {
					pkt = wlc_mbss_get_probe_template(wlc, bsscfg);
				}
				if (pkt != NULL) {
...
wlc_ap_scb_free_notify(void *context, struct scb *scb)
{
	...
	/* mark the aid unused */
	if (scb->aid && bsscfg->aidmap) {
		ASSERT(AID2AIDMAP(scb->aid) < wlc->pub->tunables->maxscb);
		clrbit(bsscfg->aidmap, AID2AIDMAP(scb->aid));
	}
}
...
#if defined(TXQ_MUX)
/**
 * @brief Inlined cubby fucntions for BCMC code.
 *
...
wlc_bcmc_mux_output(void *ctx, uint ac, uint request_time, struct spktq *output_q)
{
	...
	WL_ERROR(("%s() req_time:%u  ac:%d muxq:0x%p\n",
		__FUNCTION__, request_time, ac, BCMC_MUXSRC(cfg)));
#endif
...
wlc_bcmc_mux_free(wlc_ap_info_pvt_t *appvt, wlc_bsscfg_t *cfg)
{
	if (BCMC_MUXSRC(appvt, cfg)) {
		 ...
		 */
		ASSERT(pktq_empty(BCMC_QUEUE(appvt, cfg)));

		wlc_msrc_free(cfg->wlc, cfg->wlc->osh, BCMC_MUXSRC(appvt, cfg));
		wlc_bcmc_mux_queue_deinit(BCMC_QUEUE(appvt, cfg));
