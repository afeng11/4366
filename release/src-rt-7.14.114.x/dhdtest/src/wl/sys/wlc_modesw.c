...
static void wlc_modesw_sta_opmode_timer(void *arg);
...
wlc_modesw_alloc_context(wlc_modesw_info_t *modesw_info, wlc_bsscfg_t *cfg)
{
	...
	if (pmodesw_bsscfg->timer_ctx == NULL) {
		...
		if (BSSCFG_AP(cfg)) {
			...
			pmodesw_bsscfg->oper_mode_timer =
			wl_init_timer(wlc->wl, wlc_modesw_sta_opmode_timer,
			pmodesw_bsscfg->timer_ctx, "opermode_sta_dgrade_timer");
		}
...
static void
wlc_modesw_sta_opmode_timer(void *arg)
{
	...
	/* In case bsscfg is freed before this callback is invoked */
	WL_MODE_SWITCH(("wl%d: STA Opmode Timer Expired for cfg = %d\n",
		WLCWLUNIT(wlc), bsscfg->_idx));
	wlc_modesw_oper_mode_complete(wlc->modesw, bsscfg, BCME_ERROR, MODESW_STA_TIMEOUT);
	return;
}
	...
	uint8 oper_mode, uint8 enabled)
{
	...
	if (!wlc_modesw_is_downgrade(wlc, bsscfg, bsscfg->oper_mode, oper_mode)) {
		...
		wlc_modesw_notif_cb_notif(modesw_info, bsscfg,
			BCME_OK, oper_mode, MODESW_PHY_UP_COMPLETE);
