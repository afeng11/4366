 ...
 *
 * $Id: wlc_wnm.c 526409 2015-01-14 06:12:12Z $
 */
...
static void wlc_wnm_send_bsstrans_request(wlc_wnm_info_t *wnm, wlc_bsscfg_t *bsscfg,
	struct scb *scb, uint8 token, uint8 reqmode);
...
BCMATTACHFN(wlc_wnm_attach)(wlc_info_t *wlc)
{
	...
	if (wlc_iem_vs_add_parse_fn_mft(wlc->iemi, parse_fstbmp, WLC_IEM_VS_IE_PRIO_BRCM,
		...
		WL_ERROR(("wl%d: %s wlc_iem_vs_add_parse_fn failed, brcm ie in wnm assocreq\n",
			wlc->pub->unit, __FUNCTION__));
		goto fail;
	}
#endif /* WLWNM_BRCM */
	...
	uint8 *body, int body_len)
{
	...
	case DOT11_WNM_ACTION_BSSTRANS_QUERY: {
		...
		wlc_wnm_send_bsstrans_request(wnm, bsscfg, scb, transquery->token, reqmode);
		break;
	}
...
wlc_wnm_bsscfg_init(void *context, wlc_bsscfg_t *cfg)
{
...
#ifdef WNM_TFS
	/* create default NULL TFS Filter */
	wnm_tclas_t *tclas;

	tclas = (wnm_tclas_t *)MALLOCZ(wlc->osh, TCLAS_ELEM_FIXED_SIZE + DOT11_TCLAS_FC_0_ETH_LEN);
	if (tclas == NULL) {
		WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
			wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
		err = BCME_ERROR;
	...
	tclas->fc.hdr.type = DOT11_TCLAS_FC_0_ETH;
	tclas->fc.hdr.mask = 0;
	tclas->next = NULL;
	/* Get the user priority */
	tclas->user_priority = 0;
	tclas->fc_len = DOT11_TCLAS_FC_0_ETH_LEN;

	WNM_ELEMENT_LIST_ADD(wnm->tclas_head, tclas);
	wnm->tclas_cnt++;

	wl_tfs_set_t tfs = {0, 1, 0, 0};
	wlc_wnm_tfs_set(cfg, wnm_cfg, &tfs);
#endif /* WNM_TFS */
...
wlc_wnm_tttt(wlc_info_t *wlc)
{
	...
	FOREACH_UP_AP(wlc, idx, bsscfg) {
		...
		if (wnm_cfg->timbc_frame.p != NULL) {
			...
			if (dma_txfast(WLC_HW_DI(wlc, TX_ATIM_FIFO),
...
{
#ifdef WLWNM_AP
	wnm_dms_scb_t *dms_scb = dms_desc->dms_scb_head;
	while (dms_scb) {
...
wlc_wnm_dms_req_parse_resp_send(wlc_bsscfg_t *bsscfg, struct scb *scb, uint8 *req_body, int req_len)
{
	...
	while (req_offset != req_len) {
		...
		while (req_ie_offset != req_ie->len) {
			...
			if (2 + dms_req_desc->len > req_ie->len - req_ie_offset) {
...
wlc_wnm_bsstrans_req_timer(void *arg)
{
	...
	case BSSTRANS_WAIT_FOR_BSS_TERM:
		if (bsstrans->status == (1 << DOT11_BSSTRANS_RESP_STATUS_ACCEPT)) {
			struct scb_iter scbiter;
			struct scb *scb = NULL;
			...
			if (bsstrans->status ==
			    (1 << DOT11_BSSTRANS_RESP_STATUS_REJ_TERM_DELAY_REQ |
			    1 << DOT11_BSSTRANS_RESP_STATUS_ACCEPT) &&
			    bsstrans->delay > 0) {
	...
	uint8 token, uint8 reqmode)
{
	...
	if (reqmode & DOT11_BSSTRANS_REQMODE_PREF_LIST_INCL)
		maxlen += bsstrans->bsstrans_list_len;
	...
	bool periodic)
{
	...
	bcm_l2_filter_arp_table_update(wlc->osh, ptable, all, del_ea, periodic, wlc->pub->now);
}
...
wlc_wnm_enter_sleep_mode(wlc_bsscfg_t *cfg)
{
	...
	if (!wnm_cfg->bss_max_idle_period) {
		WL_ERROR(("wl%d: %s: BSS Max Idle Period must be set by AP\n",
			wlc->pub->unit, __FUNCTION__));
		return BCME_ERROR;
	}
		...
		if (reply != NULL) {
			if (reply_to_bss) {
				...
				WL_WNM(("wl%d: sendup proxy-reply frame\n",
					WLC_BSSCFG_IDX(bsscfg)));
				wlc_sendup(wlc, bsscfg, NULL, reply);
			}
...
wlc_wnm_bss_pref_score_rssi(wlc_info_t *wlc, wlc_bss_info_t *bi, int8 bcn_rssi, uint32 *score)
{
	...
	if (is_ht) {
		for (i = RSSI_RATE_MAP_MAX_STREAMS; i > 0; i--) {
			if (bi->rateset.mcs[i - 1] && i <= wlc->stf->txstreams) {
				nss = i;
				break;
			}
		}
