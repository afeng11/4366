 ...
 *
 * $Id: wlc_rate_sel.c 528293 2015-01-21 23:18:59Z $
 */
...
wlc_ratesel_dump_rateset(rcb_t *state, struct bcmstrbuf *b)
{
	...
	if (b) {
		...
		if (state->active_antcfg_num >= 1)
			bcm_bprintf(b, " bwcap %d spmode %d sgi_bwen %d antsel ON antcfg %d",
				state->bwcap, state->spmode, state->sgi_bwen,
				state->active_antcfg_num);
		else
			bcm_bprintf(b, " bwcap %d spmode %d sgi_bwen %d antsel OFF",
				state->bwcap, state->spmode, state->sgi_bwen);
#endif /* WL11N */
	...
	if (use_hist) {
		...
		if (dnp->psr <= cur->psr)
			...
			psr_up = (cur->psr * (prate_up - prate_dn)
				  - dnp->psr * (prate_up - prate_cur)) / (prate_cur - prate_dn);
		}
	} else {
