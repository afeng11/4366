 ...
 *
 * $Id: wlc_akm_ie.c 528293 2015-01-21 23:18:59Z $
 */
...
wlc_rsn_build_ie(wlc_info_t *wlc, wlc_bsscfg_t *cfg, struct rsn_parms *rsn, void *pmcast)
{
	...
	if (WSEC_AES_ENABLED(cfg->wsec) && !UCAST_NONE(rsn))
		ucast->list[0].type = WPA_CIPHER_AES_CCM;
	else if (!WSEC_AES_ENABLED(cfg->wsec) &&
		WSEC_TKIP_ENABLED(cfg->wsec) && !UCAST_NONE(rsn))
	...
	bool reassoc, uint8 *ie, uint8 *buf, uint len)
{
	...
	if (wpa2_ie != NULL &&
		...
		if (
			...
			!(fbt && (reassoc == 0)) &&
#endif /* WLFBT */
			bcmwpa_is_wpa2_auth(cfg->WPA_auth)) {
	/* use cached PMKIDs */
...
wlc_akm_arq_calc_rsn_ie_len(void *ctx, wlc_iem_calc_data_t *data)
{
	...
	if (bcmwpa_is_wpa2_auth(cfg->WPA_auth) && WSEC_ENABLED(cfg->wsec)) {
