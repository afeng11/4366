 ...
 *
 * $Id: wlc_phy_int.h 610865 2016-01-08 03:13:40Z $
 */
...
#endif
#define RADIO2069_MAJORREV(rev)	((RADIO2069REV(rev) == 0x40) ? 0:(RADIO2069REV(rev) >> 4))
#define RADIO2069_MINORREV(rev)	((RADIO2069REV(rev) == 0x40) ? 16:(RADIO2069REV(rev) & 0x0fU))
/*
 * For the 20691 Radio:
...
#define RADIO20691_MAJORREV(rev)	((RADIO20691REV(rev) <= 0x1A) ? 0 : \
					 (RADIO20691REV(rev) <= 0x52) ? 1 : \
					 (RADIO20691REV(rev) == 0x81) ? 1 : 2)
#define RADIO20691_MINORREV(rev)	((RADIO20691_MAJORREV(rev) == 0) ? \
					...
					(RADIO20691_MAJORREV(rev) == 1) ? \
					 ((RADIO20691REV(rev) <= 0x25) ? 0 : \
					  ...
					  (RADIO20691REV(rev) <= 0x52) ? 4 : \
					  (RADIO20691REV(rev) == 0x81) ? 16 : 5) : 0)
...
#define RADIO20693_MINORREV(rev)	((RADIO20693_MAJORREV(rev) == 0) ? 0 : \
	...
	(RADIO20693REV(rev) == 0x11) ? 3 : (RADIO20693REV(rev) == 0x0F) ? 4 : 5) : 0)
...
struct phy_info
{
	...
	int8	phy_spuravoid;			/* spur avoidance
	...
	uint16	tbl_data_wide;			/* Phy table addr/data register offsets */
	uint16	tbl_data_hi;
	uint16	tbl_data_lo;
	uint16	tbl_addr;

	uint8	a_band_high_disable;		/* high channels in A band disabled for srom v1 */
	uint8	*hwtxpwr;			/* tssi to dbm translation table */
	int8	nphy_rssisel;
	...
	uint	itssi_war_cnt;
	uint16	tx_alpf_bypass;			/* bypass tx analog lpf */
	uint16 	tx_alpf_bypass_2g;		/* bypass tx analog lpf */
	uint16 	tx_alpf_bypass_5g;		/* bypass tx analog lpf */
	...
	uint8	fast_bs; 	/* This is used to bypass some of the initialization in the init
				 * routine to save band switch time.
				 * This needs to be reset on a phy reset
				 */
	...
	uint8	papdcal_indexdelta;			/* txindex delta below which papd
	...
	/* #ifdef PR43338WAR */
	bool	war_b_ap;				/* b only AP detected */
...
extern void wlc_acphy_print_phydbg_regs(phy_info_t *pi);
#endif /* DNG_DBGDUMP */
...
extern uint8 wlc_phy_rssi_get_chan_freq_range_acphy(phy_info_t *pi, uint8 core_segment_mapping);
...
extern void wlc_phy_adjust_ed_thres_lcn40phy(phy_info_t *pi,
	int32 *assert_thresh_dbm, bool set_threshold);
extern uint32 wlc_phy_rx_iq_est(phy_info_t *pi, uint8 samples, uint8 antsel, uint8 resolution,
	uint8 lpf_hpc, uint8 dig_lpf, uint8 gain_correct,
                                uint8 extra_gain_3dB, uint8 wait_for_crs, uint8 force_gain_type);
extern int wlc_phy_set_interference_override_mode(phy_info_t* pi, int val);
...
extern int wlc_phy_tof_kvalue_acphy(phy_info_t *pi, chanspec_t chanspec, ratespec_t rspec,
