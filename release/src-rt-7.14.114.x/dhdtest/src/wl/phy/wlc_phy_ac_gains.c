 ...
 *
 * $Id: wlc_phy_ac_gains.c 527374 2015-01-17 05:11:33Z $
 */
...
wlc_phy_tx_gain_table_write_acphy(phy_info_t *pi, uint32 l, uint32 o, uint32 w, const void *d)
{
	...
	FOREACH_CORE(pi, core) {
		...
		if ((!ACMAJORREV_4(pi->pubpi->phy_rev)) && (core != 0)) {
			continue;
		}
...
BCMATTACHFN(wlc_phy_set_txgain_tbls)(phy_info_t *pi)
{
	...
	if (RADIOID_IS(pi->pubpi->radioid, BCM20691_ID)) {
		...
		tx_pwrctrl_tbl = (PHY_IPA_ATTACH_5G_PARAMS(pi)) ? txgain_20693_phyrev12_5g_ipa
