 ...
 *
 * $Id: wlc_phy_lcn.c 527374 2015-01-17 05:11:33Z $
 */
...
wlc_lcnphy_papd_cal_txpwr(phy_info_t *pi)
{
	...
	if ((CHIPID(pi->sh->chip) == BCM4336_CHIP_ID) &&
		...
		if (CHIPREV(pi->sh->chiprev) == 0)
			txgains.index = 60;
		else if (CHIPREV(pi->sh->chiprev) == 1)
			txgains.index = 40;
		else
			txgains.index = 55;
	}
...
/* const attribute of lcnphy_tbl_ptr[idx].tbl_ptr is discarded intentionally
 *  as data cloned from ROM to RAM
 */
...
/* tbl_ptr defined const, but allocated if cloned to RAM, so const attr should be
 * discarded, suppress -Wcast-qual warning
 */
#if defined(STRICT_GCC_WARNINGS) && defined(__GNUC__) && (__GNUC__ > 4 || (__GNUC__ == \
	4 && __GNUC_MINOR__ >= 6))
_Pragma("GCC diagnostic push")
...
BCMATTACHFN(wlc_phy_txpwr_srom_read_lcnphy)(phy_info_t *pi, int bandtype)
{
	...
	for (i = 1; i <= CH_MAX_2G_CHANNEL; i++) {
		(void)snprintf(str1, sizeof(str1), rstr_openlpgainidxbN, i);
...
#ifdef BAND5G
	for (i = 0; i <= 4; i++) {
		(void)snprintf(str1, sizeof(str1), rstr_logen_mode5gx, i);
...
wlc_phy_txpwr_sromlcn_read_2g_ppr_parameters(phy_info_t *pi)
{
	...
	pi->ppr->u.sr_lcn.cck2gpo = (uint16)PHY_GETINTVAR(pi, rstr_cck2gpo);

	/* Extract offsets for 8 OFDM rates */
	pi->ppr->u.sr_lcn.ofdm = (uint32)PHY_GETINTVAR(pi, rstr_ofdm2gpo);

	/* Extract offsets for 8 MCS rates */
	/* mcs2gpo(x) are 16 bit numbers */
	pi->ppr->u.sr_lcn.mcs = ((uint16)PHY_GETINTVAR(pi, rstr_mcs2gpo1) << 16) |
...
{
	srom_lcn_ppr_t *sr_lcn = &pi->ppr->u.sr_lcn;
...
wlc_lcnphy_radio_2064_channel_tune(phy_info_t *pi, uint8 channel)
{
	...
	/* Turn on PLL power supplies */
	if (((CHSPEC_IS2G(pi->radio_chanspec)) &&
		(pi->u.pi_lcnphy->rfpll_doubler_2g == TRUE)) ||
		((CHSPEC_IS5G(pi->radio_chanspec)) &&
		(pi->u.pi_lcnphy->rfpll_doubler_5g == TRUE)))
		rfpll_doubler = 1;
...
wlc_lcnphy_papd_recal(phy_info_t *pi)
{
		...
		(CHIPID(pi->sh->chip) == BCM4313_CHIP_ID) && !pi_lcn->ePA)
	{
		...
		pi_lcn->tx_pwr_idx[CHSPEC_CHANNEL(pi->radio_chanspec)] =
			pi_lcn->lcnphy_start_idx;
#endif  /* MULTICHAN_4313 */
