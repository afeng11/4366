...
static int32 wlc_phy_tempsense_poll_samps_tiny(phy_info_t *pi,
	uint16 samples, bool init_adc_inside);
...
static int32 wlc_phy_tempsense_paldosense_phy_cleanup_acphy_tiny(phy_info_t *pi);
...
wlc_phy_tempsense_paldosense_phy_setup_acphy_tiny(phy_info_t *pi, uint8 tempsense_paldosense)
{
	...
	MOD_PHYREGCE(pi, RfctrlOverrideAuxTssi,  0, afe_iqadc_aux_en, 1);
	MOD_PHYREGCE(pi, RfctrlCoreAuxTssi1,	 0, afe_iqadc_aux_en, 1);
	MOD_PHYREGCE(pi, RfctrlOverrideRxPus,	 0, lpf_pu_dc, 1);
	MOD_PHYREGCE(pi, RfctrlCoreRxPus,		 0, lpf_pu_dc, 0);
	MOD_PHYREG(pi, RxFeCtrl1, swap_iq0, 0);
	MOD_PHYREGCE(pi, RfctrlOverrideTxPus,    0, lpf_bq1_pu, 1);
	MOD_PHYREGCE(pi, RfctrlCoreTxPus,        0, lpf_bq1_pu, 1);
	MOD_PHYREGCE(pi, RfctrlOverrideTxPus,    0, lpf_bq2_pu, 1);
	MOD_PHYREGCE(pi, RfctrlCoreTxPus,        0, lpf_bq2_pu, 0);
	MOD_PHYREGCE(pi, RfctrlOverrideTxPus,    0, lpf_pu, 1);
	MOD_PHYREGCE(pi, RfctrlCoreTxPus,        0, lpf_pu, 1);
	MOD_PHYREGCE(pi, RfctrlOverrideGains,    0, lpf_bq1_gain, 1);
	MOD_PHYREGCE(pi, RfctrlCoreLpfGain,      0, lpf_bq1_gain, 0);

		MOD_PHYREGCE(pi, RfctrlOverrideAuxTssi,  0, amux_sel_port, 1);
	...
	if (!(tempsense_paldosense)) {
		...
		MOD_PHYREGCE(pi, RfctrlOverrideRxPus,	 0, rxrf_lna1_pwrup, 1);
		MOD_PHYREGCE(pi, RfctrlOverrideRxPus,	 0, rxrf_lna1_5G_pwrup, 1);
		MOD_PHYREGCE(pi, RfctrlCoreRxPus,	 0, rxrf_lna1_pwrup, 0);
		MOD_PHYREGCE(pi, RfctrlCoreRxPus,	 0, rxrf_lna1_5G_pwrup, 0);
		if (ACMAJORREV_4(pi->pubpi->phy_rev))
...
static int32
wlc_phy_tempsense_paldosense_phy_cleanup_acphy_tiny(phy_info_t *pi)
{
	phy_info_acphy_t *pi_ac = (phy_info_acphy_t *)pi->u.pi_acphy;
	acphy_tempsense_phyregs_t *porig = (pi_ac->tempi->ac_tempsense_phyregs_orig);
	uint8 core = 0;
...
wlc_phy_tempsense_paldosense_acphy_tiny(phy_info_t *pi, uint8 tempsense_paldosense)
{
	...
	wlc_phy_tempsense_paldosense_phy_setup_acphy_tiny(pi, tempsense_paldosense);
	...
	wlc_phy_tempsense_paldosense_phy_cleanup_acphy_tiny(pi);
	wlc_phy_tempsense_paldosense_radio_cleanup_acphy_tiny(pi, tempsense_paldosense);
...
wlc_phy_vbat_monitoring_algorithm_acphy(phy_info_t *pi)
{
	...
	uint32 ldo_data[5] = {0x88007081, 0x88077081, 0x88067081, 0x88057081, 0x88047081};
	int16 threshold = 12;
	int16 read_high_voltage;
	int16 read_new_voltage;

	/* setting the address for LDO and writing the 3p3 voltage */
	wlc_si_pmu_regcontrol_access(pi, 4, &ldo_data[0], 1);
	/* setting the address for PALDO and writing the 3p3 voltage */
	wlc_si_pmu_regcontrol_access(pi, 7, &paldo_data[0], 1);
	...
	for (i = 1; i < 5; i++) {
		/* setting the address for LDO and writing the 3p3 voltage */
		wlc_si_pmu_regcontrol_access(pi, 4, &ldo_data[i], 1);
		/* setting the address for PALDO and writing the 3p3 voltage */
		wlc_si_pmu_regcontrol_access(pi, 7, &paldo_data[i], 1);
	...
	if (i != 6) {
		/* setting the address for LDO and writing the 3p3 voltage */
		wlc_si_pmu_regcontrol_access(pi, 4, &ldo_data[i-1], 1);
		/* setting the address for PALDO and writing the 3p3 voltage */
		wlc_si_pmu_regcontrol_access(pi, 7, &paldo_data[i-1], 1);
	} else {
		/* setting the address for LDO and writing the 3p3 voltage */
		wlc_si_pmu_regcontrol_access(pi, 4, &ldo_data[i-2], 1);
		/* setting the address for PALDO and writing the 3p3 voltage */
		wlc_si_pmu_regcontrol_access(pi, 7, &paldo_data[i-2], 1);
	}
	if (i != 1)
		do_cals = 0;
