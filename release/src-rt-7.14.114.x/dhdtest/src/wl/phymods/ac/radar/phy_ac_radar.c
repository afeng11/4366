...
static int phy_ac_radar_run(phy_type_radar_ctx_t *ctx);
...
BCMATTACHFN(phy_radar_init_st)(phy_info_t *pi, phy_radar_st_t *st)
{
	...
	st->rparams.radar_args.ncontig = 45616; /* 37424;37411; */
...
WLBANDINITFN(phy_ac_radar_init)(phy_type_radar_ctx_t *ctx, bool on)
{
	...
	if (TINY_RADIO(pi)) {
		phy_utils_write_phyreg(pi, ACPHY_Radar_adc_to_dbm(pi->pubpi->phy_rev), 0x4ac);
...
static int
phy_ac_radar_run(phy_type_radar_ctx_t *ctx)
{
	phy_ac_radar_info_t *info = (phy_ac_radar_info_t *)ctx;
	phy_info_t *pi = info->pi;
