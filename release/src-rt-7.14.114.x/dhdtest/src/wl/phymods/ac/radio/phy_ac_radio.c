...
static void wlc_phy_chanspec_radio20693_setup(phy_info_t *pi, uint8 ch,
...
phy_ac_radio_switch(phy_type_radio_ctx_t *ctx, bool on)
{
	...
	if ((RADIOID_IS(pi->pubpi->radioid, BCM20693_ID)) && (pi->radio_is_on == TRUE) && (!on)) {
...
{
	uint i;
	chan_info_radio20693_pll_t *chan_info_tbl_pll;
	chan_info_radio20693_rffe_t *chan_info_tbl_rffe;
	uint32 tbl_len;
	...
	*chan_info_rffe = &chan_info_tbl_rffe[i];
	...
	return chan_info_tbl_pll[i].freq;
}

static void
wlc_phy_chanspec_radio20693_setup(phy_info_t *pi, uint8 ch, uint8 toggle_logen_reset, uint8 core)
{
	const chan_info_radio20693_pll_t *chan_info_pll;
	const chan_info_radio20693_rffe_t *chan_info_rffe;
	...
	if (wlc_phy_chan2freq_20693(pi, ch, &chan_info_pll, &chan_info_rffe) < 0)
		return;
	...
	if (!((phy_get_phymode(pi) == PHYMODE_MIMO) && (core == 1))) {
...
wlc_phy_tiny_radio_minipmu_cal(phy_info_t *pi)
{
	...
	FOREACH_CORE(pi, core) {
		...
		if ((RADIOID_IS(pi->pubpi->radioid, BCM20693_ID)) && (RADIOMAJORREV(pi) < 2) &&
	...
	FOREACH_CORE(pi, core) {
		if ((RADIOID_IS(pi->pubpi->radioid, BCM20693_ID)) &&
			(phy_get_phymode(pi) == PHYMODE_RSDB) && (RADIOMAJORREV(pi) < 2) &&
			(phy_get_current_core(pi) == PHY_RSBD_PI_IDX_CORE1)) {
		...
		if (RADIOID_IS(pi->pubpi->radioid, BCM20693_ID)) {
			FOREACH_CORE(pi, core) {
				...
				phy_utils_write_radioreg(pi, RADIO_REG_20693(pi,
					PLL_CFG1, core), data);
				MOD_RADIO_REG_TINY(pi, PLL_HVLDO1, core, ldo_2p5_pu_ldo_CP, 0);
				MOD_RADIO_REG_TINY(pi, PLL_HVLDO1, core, ldo_2p5_pu_ldo_VCO, 0);
...
wlc_phy_switch_radio_acphy_20691(phy_info_t *pi)
{
	...
	if (((RADIO20691_MAJORREV(pi->pubpi->radiorev) == 1) &&
		...
		wlc_phy_radio_tiny_rcal(pi, 0);
	} else {
		/* hard-coded RCal value */
		wlc_phy_radio_tiny_rcal(pi, 1);	wlc_phy_radio_tiny_rcal(pi, 1);
	}
#endif
}
...
void wlc_phy_radio20693_mimo_core1_pmu_off(phy_info_t *pi)
{
	...
	for (core = 0; core < pi->pubpi->phy_corenum; core++) {
		...
		for (ct = 0; ct < ARRAYSIZE(porig_offsets); ct++) {
			...
			ASSERT(ptr <= &porig->auxpga_ovr1[PHY_CORE_MAX-1]);
	...
	if (pi_ac->fc >= 0) {
		if (phy_get_phymode(pi) == PHYMODE_80P80) {
			for (n_freq_seg = 0; n_freq_seg < NUM_CHANS_IN_CHAN_BONDING;
					n_freq_seg++) {
				wlc_phy_chanspec_radio20693_setup(pi, ch[n_freq_seg],
					CCT_BAND_CHG(pi_ac) | CCT_BW_CHG(pi_ac), n_freq_seg);
			}
		} else {
			FOREACH_CORE(pi, core) {
...
chanspec_tune_radio_2069(phy_info_t *pi)
{
	...
	pi_ac->prev_subband = wlc_phy_get_chan_freq_range_acphy(pi, 0, PRIMARY_FREQ_SEGMENT);
}
...
chanspec_setup_radio(phy_info_t *pi)
{
	if (RADIOID_IS(pi->pubpi->radioid, BCM20693_ID))
		...
		PHY_ERROR(("wl%d %s: Invalid RADIOID!\n", PI_INSTANCE(pi), __FUNCTION__));
		ASSERT(0);
	}
}
