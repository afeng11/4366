	...
	phy_samp_info_t *cmn_info)
{
	...
	if ((ac_lpfCT_phyregs_orig = phy_malloc(pi, sizeof(acphy_lpfCT_phyregs_t))) == NULL) {
		PHY_ERROR(("%s: phy_malloc failed\n", __FUNCTION__));
		goto fail;
	}
	...
	/* error handling */
fail:
	if (ac_lpfCT_phyregs_orig != NULL)
		phy_mfree(pi, ac_lpfCT_phyregs_orig, sizeof(acphy_lpfCT_phyregs_t));
...
phy_ac_sample_collect(phy_type_samp_ctx_t *ctx, wl_samplecollect_args_t *collect, void *buf)
{
	...
	if (collect->trigger == TRIGGER_NOW) {
		...
		if (sampi->plast >= (collect->pre_dur + collect->post_dur) * words_per_us) {
			sampi->pfirst =	(sampi->plast -
				((collect->pre_dur + collect->post_dur) * words_per_us));
		} else {
			sampi->pfirst = ((sampi->pstop - sampi->pstart + 1) + sampi->plast -
				((collect->pre_dur + collect->post_dur) * words_per_us));
		}
	...
	if (num_cores > 1) {
		ptr[15] = htol32((((num_cores > 2) ? agc_gain[2] : 0xFFFF) << 16) | agc_gain[1]);
	} else {
