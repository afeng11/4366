	...
	phy_ana_info_t *ani)
{
	...
	if (ACPHY_FEMCTRL_ACTIVE(pi) && !wlc_phy_attach_femctrl_table(info))
		goto fail;
...
BCMATTACHFN(wlc_phy_attach_femctrl_table)(phy_ac_ana_info_t *ani)
{
	...
	pi_ac->fectrl_idx = NULL;
	pi_ac->fectrl_val = NULL;
	pi_ac->fectrl_table_len = 0;
	pi_ac->fectrl_sparse_table_len = 0;
	pi_ac->fectrl_spl_entry_flag = 0;
	...
	if (ACPHY_FEMCTRL_ACTIVE(pi)) {
		...
		case 4:
			...
			for (kk = 0; kk < sparse_table_len; kk++) {
				pi_ac->fectrl_idx[kk] = fe_ctrl_tbl[kk].idx;
				pi_ac->fectrl_val[kk] = fe_ctrl_tbl[kk].val;
			}
			pi_ac->fectrl_sparse_table_len = sparse_table_len;
			pi_ac->fectrl_table_len = table_len;
			break;
		case 7:
			/* pick size */
			if (!PHY_IPA(pi)) {
			...
			memcpy(pi_ac->fectrl_idx, fectrl_idx,
			       sparse_table_len* sizeof(uint16));
			memcpy(pi_ac->fectrl_val, fectrl_val,
			       sparse_table_len* sizeof(uint16));
			pi_ac->fectrl_sparse_table_len = sparse_table_len;
			pi_ac->fectrl_table_len = table_len;
			break;
