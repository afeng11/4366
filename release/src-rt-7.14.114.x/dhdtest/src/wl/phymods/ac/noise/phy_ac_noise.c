...
#endif /* WL_ACI_FCBS */
static void wlc_phy_hwaci_init_acphy(phy_ac_noise_info_t *noisei);
	...
	phy_noise_info_t *cmn_info)
{
	...
	phy_ac_noise_attach_params(ac_info);

#ifndef WLC_DISABLE_ACI
	/* hwaci params */
	if (!(ACPHY_ENABLE_FCBS_HWACI(pi)))
		wlc_phy_hwaci_init_acphy(ac_info);
#endif /* !WLC_DISABLE_ACI */
...
BCMATTACHFN(phy_ac_noise_attach_params)(phy_ac_noise_info_t *noisei)
{
	...
	phy_info_acphy_t *aci = noisei->aci;
...
{
	phy_info_t *pi = noisei->pi;
	phy_info_acphy_t *pi_ac = noisei->aci;
...
typedef struct _hwaci_fcbs_phytbl_list_entry {
	uint16 tbl_id;
	uint16 tbl_offset;
	uint16 num_entries;
...
wlc_phy_hwaci_mitigation_enable_acphy_tiny(phy_info_t *pi, uint8 hwaci_mode, bool init)
{
	...
	wlapi_bmac_write_shm(pi->sh->physhim, M_HWACI_ST, 0);
	...
	case HWACI_AUTO_SW:
		...
		if (init) {
			...
			if (ACMAJORREV_4(pi->pubpi->phy_rev)) {
				...
				WRITE_PHYREG(pi, Tiny_ACI_config1, 0x0514);
				WRITE_PHYREG(pi, Tiny_ACI_config2, 0x0);
				WRITE_PHYREG(pi, Tiny_ACI_config3, 0x00C8);
				WRITE_PHYREG(pi, Tiny_ACI_config4, 0x004B);
				MOD_PHYREG(pi, ACI_Detect_MAX_COUNT_HI,
					aci_detect_max_count_hi, 6);
				MOD_PHYREG(pi, ACI_Mitigation_CTRL, aci_present_th_mit_off, 0);
				MOD_PHYREG(pi, ACI_Mitigation_CTRL, aci_present_th_mit_on, 5);
			} else {
...
wlc_phy_set_aci_regs_acphy(phy_info_t *pi)
{
	...
	if (CHSPEC_IS5G(pi->radio_chanspec)) {
		if (CHSPEC_IS80(pi->radio_chanspec) && ACMAJORREV_0(pi->pubpi->phy_rev)) {
			aci_th = 0x100;
		} else {
	...
	if (0) {
		...
		if (CHSPEC_IS2G(pi->radio_chanspec) && CHSPEC_IS20(pi->radio_chanspec)) {
			...
			MOD_PHYREG(pi, RxControl, bphyacidetEn, 1);
...
wlc_phy_desense_aci_reset_params_acphy(phy_info_t *pi, bool call_gainctrl, bool all2g, bool all5g)
{
	...
	if (all2g)
		bzero(pi_ac->noisei->aci_list2g,
			ACPHY_ACI_CHAN_LIST_SZ * sizeof(acphy_aci_params_t));
	if (all5g)
		bzero(pi_ac->noisei->aci_list5g,
			ACPHY_ACI_CHAN_LIST_SZ * sizeof(acphy_aci_params_t));
