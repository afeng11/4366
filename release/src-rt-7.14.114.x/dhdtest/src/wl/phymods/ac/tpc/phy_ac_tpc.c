...
#define ESTPWRSHFTLUTS_TBL_OFFSET	0
	...
	flag2rangeon =
		((CHSPEC_IS2G(pi->radio_chanspec) && pi_ac->srom_tworangetssi2g) ||
		(CHSPEC_IS5G(pi->radio_chanspec) && pi_ac->srom_tworangetssi5g)) && PHY_IPA(pi);
	...
	*/
	...
	FOREACH_CORE(pi, core) {
		...
		if (CHSPEC_IS2G(pi->radio_chanspec) && TINY_RADIO(pi)) {
			int16 cckPwrOffset = pi->cckpwroffset[core];

			if (READ_PHYREGFLD(pi, perPktIdleTssiCtrlcck, base_index_cck_en) != 1) {
				cckPwrOffset -= pi->sh->cckPwrIdxCorr;
			}
	...
	FOREACH_ACTV_CORE(pi, pi->sh->phyrxchain, core) {
		init_idx_carry_from_lastchan = (CHSPEC_IS5G(pi->radio_chanspec)) ?
		        pi_ac->pwr_ctrl_save->status_idx_carry_5g[core]
		        : pi_ac->pwr_ctrl_save->status_idx_carry_2g[core];
		if (!init_idx_carry_from_lastchan) {
			...
			if (CHSPEC_IS5G(pi->radio_chanspec)) {
				pi_ac->pwr_ctrl_save->status_idx_carry_5g[core] = TRUE;
			} else {
				pi_ac->pwr_ctrl_save->status_idx_carry_2g[core] = TRUE;
			}
		...
		/* set power index initial condition */
			...
			if (CHSPEC_IS5G(pi->radio_chanspec)) {
				iidx = pi_ac->pwr_ctrl_save->status_idx_5g[core];
				prev_target_qdbm = pi_ac->pwr_ctrl_save->pwr_qdbm_5g[core];
				if ((pi_ac->pwr_ctrl_save->stored_not_restored_5g[core])) {
					pi_ac->pwr_ctrl_save->stored_not_restored_5g[core] = FALSE;
					pi_ac->txpwrindex_hw_save[core] = 128;
				}
			} else {
				iidx = pi_ac->pwr_ctrl_save->status_idx_2g[core];
				prev_target_qdbm = pi_ac->pwr_ctrl_save->pwr_qdbm_2g[core];
				if ((pi_ac->pwr_ctrl_save->stored_not_restored_2g[core])) {
	...
	if (ACMAJORREV_2(pi->pubpi->phy_rev) || ACMAJORREV_5(pi->pubpi->phy_rev)) {
		...
		if (CHSPEC_IS2G(pi->radio_chanspec)) {
			if (pi_ac->srom_2g_pdrange_id >= 5) {
				tssi_delay = SAMPLE_TSSI_AFTER_200_SAMPLES;
			} else if (pi_ac->srom_2g_pdrange_id >= 4) {
				tssi_delay = SAMPLE_TSSI_AFTER_220_SAMPLES;
			} else {
				tssi_delay = SAMPLE_TSSI_AFTER_150_SAMPLES;
			}
		} else {
			if (pi_ac->srom_5g_pdrange_id >= 5) {
				tssi_delay = SAMPLE_TSSI_AFTER_200_SAMPLES;
			} else if (pi_ac->srom_5g_pdrange_id >= 4) {
				tssi_delay = SAMPLE_TSSI_AFTER_220_SAMPLES;
			} else {
				tssi_delay = SAMPLE_TSSI_AFTER_150_SAMPLES;
			}
	...
	if (BF3_TSSI_DIV_WAR(pi_ac) && ACMAJORREV_1(pi->pubpi->phy_rev)) {
		...
		if ((CHSPEC_IS80(pi->radio_chanspec) || CHSPEC_IS40(pi->radio_chanspec)) &&
			 ...
			 */
			idx_set[0] = 2; idx_set[1] = 0;
		} else if ((CHSPEC_IS80(pi->radio_chanspec) || CHSPEC_IS40(pi->radio_chanspec)) &&
		    CHSPEC_IS5G(pi->radio_chanspec) && (ACMAJORREV_4(pi->pubpi->phy_rev))) {
			/* core 2/3 contains 40-80mhz paparam
			 * core 0/1 contains 20mhz paparam
			 */
		...
		FOREACH_ACTV_CORE(pi, pi->sh->phyrxchain, core) {
			...
			if (ACMAJORREV_4(pi->pubpi->phy_rev)) {
				...
				estPwrLutReg24bit = (uint32 *)phy_malloc(pi, ESTPWRLUTS_TBL_LEN*4);
				if (estPwrLutReg24bit == NULL) {
					PHY_ERROR(("%s: phy_malloc failed\n", __FUNCTION__));
				} else {
					...
					Tx Power control multi mode is set to 4 for 80P80 case
					*/
					for (idx = 0; idx < ESTPWRLUTS_TBL_LEN; idx++) {
						estPwrLutReg24bit[idx] =
							((((uint32)regval[idx]&0xFF00) << 8) |
								(uint32)regval[idx]);
					}
					/* Est Pwr Table is 128x24 Table. Limit Write to 32 bits */
					wlc_phy_table_write_acphy(pi, ACPHY_TBL_ID_ESTPWRLUTS(core),
						ESTPWRLUTS_TBL_LEN, ESTPWRLUTS_TBL_OFFSET,
						32, estPwrLutReg24bit);
		...
		if (CHSPEC_IS5G(pi->radio_chanspec)) {
			...
			if (CHIPID(pi->sh->chip) == BCM4345_CHIP_ID ||
				...
				if (ACMAJORREV_4(pi->pubpi->phy_rev)) {
					if (idx == 3) {
						pdoffs = 0;
						/* for now temp based offst support is not added */
						FOREACH_ACTV_CORE(pi, pi->sh->hw_phyrxchain,
							core) {
							core_freq_segment_map =
							pi->u.pi_acphy->core_freq_mapping[core];
							chan_freq_range =
							wlc_phy_get_chan_freq_range_acphy(pi,
									0, PRIMARY_FREQ_SEGMENT);
							pdoffs = wlc_phy_pdoffset_cal_acphy(pdoffs,
	...
	if (ACMAJORREV_4(pi->pubpi->phy_rev)) {
		...
		*/
		...
		wlc_phy_table_write_acphy(pi, ACPHY_TBL_ID_ESTPWRSHFTLUTS, ESTPWRSHFTLUTS_TBL_LEN,
...
wlc_phy_tone_pwrctrl(phy_info_t *pi, int8 tx_idx, uint8 core)
{
	...
	uint8 band, core_freq_segment_map;
	...
	*/
	...
	band = wlc_phy_get_chan_freq_range_acphy(pi, 0, core_freq_segment_map);
		...
		*/
		...
		if (!init_idx_carry_from_lastchan) {
			...
			if ((pi->u.pi_acphy->pwr_ctrl_save->stored_not_restored_2g[core])) {
			    pi->u.pi_acphy->pwr_ctrl_save->stored_not_restored_2g[core] = FALSE;
			    pi_ac->txpwrindex_hw_save[core] = 128;
...
wlc_phy_txpower_recalc_target_acphy(phy_info_t *pi)
{
	...
	if (ACREV_IS(pi->pubpi->phy_rev, 2)) {
		chan_freq_range = wlc_phy_get_chan_freq_range_acphy(pi, 0, PRIMARY_FREQ_SEGMENT);
...
wlc_phy_txpwrctrl_setminpwr(phy_info_t *pi)
{
	if (ISACPHY(pi)) {
		if (ACMAJORREV_2(pi->pubpi->phy_rev)) {
			...
			pi->min_txpower = PHY_TXPWR_MIN_ACPHY_4349;
			pi->min_txpower_5g = PHY_TXPWR_MIN_ACPHY_4349_5G;
		} else {
                                        ...
                                        ppr_t *max_pwr, uint8 core)
{
	...
	*/
	...
	band = wlc_phy_get_chan_freq_range_acphy(pi, chanspec, core_freq_segment_map);
