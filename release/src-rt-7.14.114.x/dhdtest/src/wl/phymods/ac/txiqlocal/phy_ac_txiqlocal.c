...
wlc_phy_txcal_radio_setup_acphy_tiny(phy_info_t *pi)
{
	...
	FOREACH_ACTV_CORE(pi, pi->sh->phyrxchain, core) {
		...
		/* # Enabling and Muxing per band */
			...
			/* #tap from PA output */
			MOD_RADIO_REG_TINY(pi, TX5G_MISC_CFG1, core, pa5g_tssi_ctrl_sel, 0);
			MOD_RADIO_REG_TINY(pi, TX_TOP_5G_OVR1, core, ovr_pa5g_tssi_ctrl_sel, 0x1);
		}
		...
		MOD_RADIO_REG_TINY(pi, IQCAL_CFG1, core, iqcal_PU_tssi, 0x0); /* # power off tssi */
	...
	if (en_precal_gain_control == 0) {
		...
		FOREACH_ACTV_CORE(pi, pi->sh->phyrxchain, core) {
			...
			if (ACREV_IS(pi->pubpi->phy_rev, 1) || ACMAJORREV_5(pi->pubpi->phy_rev)) {
				...
				subband = wlc_phy_get_chan_freq_range_acphy(pi,
...
wlc_phy_cal_txiqlo_acphy(phy_info_t *pi, uint8 searchmode, uint8 mphase, uint8 Biq2byp)
{
		...
		if (TINY_RADIO(pi)) {
			...
			if (ACMAJORREV_4(pi->pubpi->phy_rev)) {
				if (ACMINORREV_2(pi) && (CHSPEC_IS5G(pi->radio_chanspec))) {
					...
					cmds_RESTART[2] = 0x085; cmds_RESTART[3] = 0x033;

					cmds_REFINE[0] = 0x264; cmds_REFINE[1] = 0x233;
					cmds_REFINE[2] = 0x085; cmds_REFINE[3] = 0x033;
				} else {
	...
	if (TINY_RADIO(pi)) {
		...
		if (ACMAJORREV_4(pi->pubpi->phy_rev)) {
			wlc_phy_ipa_set_bbmult_acphy(pi, &(accal->txcal_txgain[0].bbmult),
				&(accal->txcal_txgain[1].bbmult), NULL, pi->pubpi->phy_coremask);
		} else {
			wlc_phy_ipa_set_bbmult_acphy(pi, &(accal->txcal_txgain[0].bbmult),
				NULL, NULL, pi->pubpi->phy_coremask);
		}
	}
	...
	if (TINY_RADIO(pi) && ACPHY_IBOARD(pi)) {
		OSL_DELAY(10000);
	}
	/* ---------------
	 *  Cmd Execution
