...
wlc_phy_desense_mf_high_thresh_acphy(phy_info_t *pi, bool on)
{
	...
	if (ACREV_GE(pi->pubpi->phy_rev, 32)) {
		WRITE_PHYREG(pi, crshighlowpowThresholdl0, val);
		WRITE_PHYREG(pi, crshighlowpowThresholdu0, val);
		WRITE_PHYREG(pi, crshighlowpowThresholdlSub10, val);
		WRITE_PHYREG(pi, crshighlowpowThresholduSub10, val);
		WRITE_PHYREG(pi, crshighlowpowThresholdl1, val);
		WRITE_PHYREG(pi, crshighlowpowThresholdu1, val);
		WRITE_PHYREG(pi, crshighlowpowThresholdlSub11, val);
		WRITE_PHYREG(pi, crshighlowpowThresholduSub11, val);
		WRITE_PHYREG(pi, crshighlowpowThresholdl2, val);
		WRITE_PHYREG(pi, crshighlowpowThresholdu2, val);
		WRITE_PHYREG(pi, crshighlowpowThresholdlSub12, val);
		WRITE_PHYREG(pi, crshighlowpowThresholduSub12, val);
		WRITE_PHYREG(pi, crshighlowpowThresholdl3, val);
		WRITE_PHYREG(pi, crshighlowpowThresholdu3, val);
		WRITE_PHYREG(pi, crshighlowpowThresholdlSub13, val);
		WRITE_PHYREG(pi, crshighlowpowThresholduSub13, val);
	} else {
	...
	bool trloss, bool lna1byp, uint8 *gidx)
{
	...
	ASSERT(core < PHY_CORE_MAX);
	if (lna1byp) {
		tr = pi_ac->rxgcrsi->fem_rxgains[core].trloss;
		lna1mingain = pi_ac->rxgcrsi->rxgainctrl_params[core].gaintbl[1][0];
		lna1maxgain = pi_ac->rxgcrsi->rxgainctrl_params[core].gaintbl[1][5];
	...
	for (i = 0; i < ACPHY_MAX_RX_GAIN_STAGES; i++)
		min_gains[i] = pi_ac->rxgcrsi->rxgainctrl_params[core].gaintbl[i][0];
...
wlc_phy_rxgainctrl_gainctrl_acphy_tiny(phy_info_t *pi)
{
	...
	FOREACH_CORE(pi, core) {
		lna1byp = pi_ac->rxgcrsi->fem_rxgains[core].lna1byp; /* not supported on Tiny yet */
...
wlc_phy_desense_apply_acphy(phy_info_t *pi, bool apply_desense)
{
	...
	*/
	...
	uint8 crsmin_thresh, crsmin_init, crsmin_high;
	...
	if (!apply_desense && !desense->forced) {
		...
		*/
		...
		/* crs high_gain */
		crsmin_thresh = ACPHY_CRSMIN_DEFAULT + ((88 * crsmin_high) >> 5);  /* hi gain */
...
wlc_phy_desense_btcoex_acphy(phy_info_t *pi, int32 mode)
{
	...
	case 7: /* Case added for 4359 */
		...
		desense->mixer_setting_desense = 4; /* Value should be take between 2-11 for Tiny */
		break;
...
wlc_phy_rxgainctrl_gainctrl_acphy(phy_info_t *pi)
{
	...
	FOREACH_CORE(pi, core) {
		...
		elna_idx = READ_PHYREGFLDC(pi, InitGainCodeA, core, initExtLnaIndex);
		max_himd_hi_end =
			- 16 - pi_ac->rxgcrsi->rxgainctrl_params[core].gaintbl[0][elna_idx];
		if (pi_ac->curr_desense->elna_bypass == 1)
			max_himd_hi_end += pi_ac->rxgcrsi->fem_rxgains[core].trloss;
...
wlc_phy_upd_lna1_lna2_gaintbls_acphy(phy_info_t *pi, uint8 lna12)
{
	...
	FOREACH_CORE(pi, core) {
		...
		memcpy(pi_ac->rxgcrsi->rxgainctrl_params[core].gaintbl[lna12],
			gaintbl, sizeof(uint8)*sz);
		wlc_phy_table_write_acphy(pi, gain_tblid, sz, offset, 8, gaintbl);
		memcpy(pi_ac->rxgcrsi->rxgainctrl_params[core].gainbitstbl[lna12], gainbitstbl,
			sizeof(uint8)*sz);
	...
	if (!TINY_RADIO(pi)) {
		...
		FOREACH_CORE(pi, core) {
			...
			wlc_phy_table_write_acphy(pi, gainlimitid, lna2_limit_size,
				16, 8, tiny_lna2_tbl);
			wlc_phy_table_write_acphy(pi, gainlimitid, lna2_limit_size,
				16+64, 8, tiny_lna2_tbl);
			wlc_phy_table_write_acphy(pi, gainlimitid, 12, 32,    8, tiny_tia_tbl);
			wlc_phy_table_write_acphy(pi, gainlimitid, 12, 32+64, 8, tiny_tia_tbl);
		}
	}
}
...
wlc_phy_ofdm_crs_acphy(phy_info_t *pi, bool enable)
{
	...
	if (enable) {
		if (ACREV_GE(pi->pubpi->phy_rev, 32)) {
			...
			MOD_PHYREG(pi, crsControll0, totEnable, 1);
			MOD_PHYREG(pi, crsControluSub10, totEnable, 1);
			MOD_PHYREG(pi, crsControllSub10, totEnable, 1);

			MOD_PHYREG(pi, crsControlu1, totEnable, 1);
			MOD_PHYREG(pi, crsControll1, totEnable, 1);
			MOD_PHYREG(pi, crsControluSub11, totEnable, 1);
			MOD_PHYREG(pi, crsControllSub11, totEnable, 1);

			MOD_PHYREG(pi, crsControlu2, totEnable, 1);
			MOD_PHYREG(pi, crsControll2, totEnable, 1);
			MOD_PHYREG(pi, crsControluSub12, totEnable, 1);
			MOD_PHYREG(pi, crsControllSub12, totEnable, 1);

			MOD_PHYREG(pi, crsControlu3, totEnable, 1);
			MOD_PHYREG(pi, crsControll3, totEnable, 1);
			MOD_PHYREG(pi, crsControluSub13, totEnable, 1);
			MOD_PHYREG(pi, crsControllSub13, totEnable, 1);
		} else {
		...
		if (ACREV_GE(pi->pubpi->phy_rev, 32)) {
			...
			MOD_PHYREG(pi, crsControll0, totEnable, 0);
			MOD_PHYREG(pi, crsControluSub10, totEnable, 0);
			MOD_PHYREG(pi, crsControllSub10, totEnable, 0);

			MOD_PHYREG(pi, crsControlu1, totEnable, 0);
			MOD_PHYREG(pi, crsControll1, totEnable, 0);
			MOD_PHYREG(pi, crsControluSub11, totEnable, 0);
			MOD_PHYREG(pi, crsControllSub11, totEnable, 0);

			MOD_PHYREG(pi, crsControlu2, totEnable, 0);
			MOD_PHYREG(pi, crsControll2, totEnable, 0);
			MOD_PHYREG(pi, crsControluSub12, totEnable, 0);
			MOD_PHYREG(pi, crsControllSub12, totEnable, 0);

			MOD_PHYREG(pi, crsControlu3, totEnable, 0);
			MOD_PHYREG(pi, crsControll3, totEnable, 0);
			MOD_PHYREG(pi, crsControluSub13, totEnable, 0);
			MOD_PHYREG(pi, crsControllSub13, totEnable, 0);
		} else {
...
wlc_phy_crs_min_pwr_cal_acphy(phy_info_t *pi, uint8 crsmin_cal_mode)
{
	...
	FOREACH_ACTV_CORE(pi, pi->sh->phyrxchain, i)  {
		...
		*/
		core_freq_segment_map = pi->u.pi_acphy->core_freq_mapping[i];

		/* check the freq range of the current channel */
		/* 2G, 5GL, 5GM, 5GH, 5GX */
		chan_freq_range = wlc_phy_get_chan_freq_range_acphy(pi, 0, core_freq_segment_map);
		...
		if (PHY_ILNA(pi)) {
			...
			*/
			memcpy(pi_ac->rxgcrsi->rxgainctrl_params[core].gaintbl[3],
				mix_2g_43352_ilna, sizeof(uint8)*pi_ac->rxgainctrl_stage_len[3]);
		} else {
	...
	uint8 core, uint16 gain_tblid, uint16 gainbits_tblid)
{
	...
	int8 elna_gain = pi_ac->rxgcrsi->rxgainctrl_params[core].gaintbl[0][0];
	uint8 i;
	bool elna_present = CHSPEC_IS2G(pi->radio_chanspec) ? BF_ELNA_2G(pi_ac)
	                                                    : BF_ELNA_5G(pi_ac);
	...
	const uint8 used = 0;
	uint16 gainlimit_tblid = (ACMAJORREV_4(pi->pubpi->phy_rev)) ?
		((core) ? ACPHY_TBL_ID_GAINLIMIT1 : ACPHY_TBL_ID_GAINLIMIT0)
		: ACPHY_TBL_ID_GAINLIMIT;
	...
	memcpy(pi_ac->rxgcrsi->rxgainctrl_params[core].gaintbl[LNA1_ID], lna1_gain,
		sizeof(pi_ac->rxgcrsi->rxgainctrl_params[core].gaintbl[0][0])
		* pi_ac->rxgainctrl_stage_len[LNA1_ID]);

	memcpy(pi_ac->rxgcrsi->rxgainctrl_params[core].gainbitstbl[LNA1_ID], lna1_gainbits,
		sizeof(pi_ac->rxgcrsi->rxgainctrl_params[core].gainbitstbl[0][0])
		* pi_ac->rxgainctrl_stage_len[LNA1_ID]);
	...
	bool init, bool band_change, bool bw_change)
{
	...
	FOREACH_CORE(pi, core) {
		...
		/* MIX, LPF / TIA */
		if (TINY_RADIO(pi)) {
			wlc_phy_rxgainctrl_set_gaintbls_acphy_tiny(pi, core, gain_tblid,
				gainbits_tblid);
