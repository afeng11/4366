                        ...
                        uint8 wait_time, uint8 wait_for_crs, bool rxiq_cal)
{
	...
	bool percore = rxiq_cal &
		...
		? pi->u.pi_acphy->rxiqcal_percore_2g : pi->u.pi_acphy->rxiqcal_percore_5g);
	uint8 stall_val;
...
wlc_phy_rxcal_radio_setup_acphy_tiny(phy_info_t *pi)
{
	...
	FOREACH_CORE(pi, core) {
		...
		if (CHSPEC_IS2G(pi->radio_chanspec)) {
			...
			MOD_RADIO_REG_TINY(pi, RX_TOP_2G_OVR_EAST, core, ovr_gm2g_auxgm_pwrup, 0x1);
