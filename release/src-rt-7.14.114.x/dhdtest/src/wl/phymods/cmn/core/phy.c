...
BCMATTACHFN(phy_module_attach)(shared_phy_t *sh, void *regs, int bandtype, char *vars)
{
	...
	if ((pi->pubpi = MALLOCZ(sh->osh, sizeof(wlc_phy_t))) == NULL) {
		PHY_ERROR(("wl%d: %s: out of memory, malloced pubpi %d bytes\n", sh->unit,
		          __FUNCTION__, MALLOCED(osh)));
	    goto err;
	}

	if ((pi->pubpi_ro = MALLOCZ(sh->osh, sizeof(wlc_phy_t))) == NULL) {
		PHY_ERROR(("wl%d: %s: out of memory, malloced pubpi_ro %d bytes\n", sh->unit,
		          __FUNCTION__, MALLOCED(osh)));
	    goto err;
	}

	if ((pi->interf = MALLOCZ(sh->osh, sizeof(interference_info_t))) == NULL) {
		PHY_ERROR(("wl%d: %s: out of memory, malloced interf %d bytes\n", sh->unit,
		          __FUNCTION__, MALLOCED(osh)));
	    goto err;
	}

	if ((pi->pwrdet = MALLOCZ(sh->osh, sizeof(srom_pwrdet_t))) == NULL) {
		PHY_ERROR(("wl%d: %s: out of memory, malloced pwrdet %d bytes\n", sh->unit,
		          __FUNCTION__, MALLOCED(osh)));
	    goto err;
	}

	if ((pi->txcore_temp = MALLOCZ(sh->osh, sizeof(phy_txcore_temp_t))) == NULL) {
		PHY_ERROR(("wl%d: %s: out of memory, malloced txcore_temp %d bytes\n", sh->unit,
		          __FUNCTION__, MALLOCED(osh)));
	    goto err;
	}

	if ((pi->def_cal_info = MALLOCZ(sh->osh, sizeof(phy_cal_info_t))) == NULL) {
...
#ifdef ENABLE_FCBS
	if ((pi->phy_fcbs = MALLOCZ(sh->osh, sizeof(fcbs_info))) == NULL) {
	...
	if ((pi->fem2g = MALLOCZ(sh->osh, sizeof(srom_fem_t))) == NULL) {
		PHY_ERROR(("wl%d: %s: out of memory, malloced fem2g %d bytes\n", sh->unit,
		          __FUNCTION__, MALLOCED(osh)));
	    goto err;
	}

	if ((pi->fem5g = MALLOCZ(sh->osh, sizeof(srom_fem_t))) == NULL) {
...
#ifdef WLSRVSDB
	if ((pi->srvsdb_state = MALLOCZ(sh->osh, sizeof(srvsdb_info_t))) == NULL) {
	...
	if ((pi->pi_fptr = MALLOCZ(sh->osh, sizeof(phy_func_ptr_t))) == NULL) {
		PHY_ERROR(("wl%d: %s: out of memory, malloced pi_fptr %d bytes\n", sh->unit,
		          __FUNCTION__, MALLOCED(osh)));
	    goto err;
	}

	if ((pi->ppr = MALLOCZ(sh->osh, sizeof(ppr_info_t))) == NULL) {
...
/* *********************************************** */
	...
	if (pi->ppr != NULL) {
		MFREE(pi->sh->osh, pi->ppr, sizeof(ppr_info_t));
		pi->ppr = NULL;
	}

	if (pi->pi_fptr != NULL) {
		MFREE(pi->sh->osh, pi->pi_fptr, sizeof(phy_func_ptr_t));
		pi->pi_fptr = NULL;
	}
#ifdef WLSRVSDB
	if (pi->srvsdb_state != NULL) {
		MFREE(pi->sh->osh, pi->srvsdb_state, sizeof(srvsdb_info_t));
		pi->srvsdb_state = NULL;
	}
#endif /* WLSRVSDB */
	if (pi->fem5g != NULL) {
		MFREE(pi->sh->osh, pi->fem5g, sizeof(srom_fem_t));
		pi->fem5g = NULL;
	}
	if (pi->fem2g != NULL) {
		MFREE(pi->sh->osh, pi->fem2g, sizeof(srom_fem_t));
		pi->fem2g = NULL;
	}
#ifdef ENABLE_FCBS
	if (pi->phy_fcbs != NULL) {
	    MFREE(pi->sh->osh, pi->phy_fcbs, sizeof(fcbs_info));
	    pi->phy_fcbs = NULL;
	}
#endif /* ENABLE_FCBS */
	if (pi->def_cal_info != NULL) {
	    MFREE(pi->sh->osh, pi->def_cal_info, sizeof(phy_cal_info_t));
	    pi->def_cal_info = NULL;
	}
	if (pi->txcore_temp != NULL) {
	    MFREE(pi->sh->osh, pi->txcore_temp, sizeof(phy_txcore_temp_t));
	    pi->txcore_temp = NULL;
	}
	if (pi->pwrdet != NULL) {
	    MFREE(pi->sh->osh, pi->pwrdet, sizeof(srom_pwrdet_t));
	    pi->pwrdet = NULL;
	}
	if (pi->interf != NULL) {
	    MFREE(pi->sh->osh, pi->interf, sizeof(interference_info_t));
	    pi->interf = NULL;
	}

	if (pi->pubpi_ro != NULL) {
	    MFREE(pi->sh->osh, pi->pubpi_ro, sizeof(wlc_phy_t));
	    pi->pubpi_ro = NULL;
	}

	if (pi->pubpi != NULL) {
	    MFREE(pi->sh->osh, pi->pubpi, sizeof(wlc_phy_t));
	    pi->pubpi = NULL;
	}
